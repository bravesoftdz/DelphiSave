inherited frmEmpresasABonificar: TfrmEmpresasABonificar
  Left = 370
  Top = 68
  Caption = 'Mantenimiento de empresas a bonificar'
  ClientHeight = 578
  ClientWidth = 792
  Constraints.MinHeight = 450
  Constraints.MinWidth = 800
  ExplicitWidth = 800
  ExplicitHeight = 608
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnlFiltros: TPanel
    Width = 792
    Height = 1
    ExplicitWidth = 792
    ExplicitHeight = 1
  end
  inherited CoolBar: TCoolBar
    Top = 67
    Width = 792
    Bands = <
      item
        Control = ToolBar
        ImageIndex = -1
        MinHeight = 27
        Width = 788
      end>
    ExplicitTop = 67
    ExplicitWidth = 792
    inherited ToolBar: TToolBar
      Left = 9
      Width = 779
      ExplicitWidth = 779
      inherited tbModificar: TToolButton
        Enabled = True
      end
      inherited tbLimpiar: TToolButton
        Visible = True
      end
    end
  end
  inherited Grid: TArtDBGrid
    Top = 96
    Width = 792
    Height = 482
    Columns = <
      item
        Expanded = False
        FieldName = 'EB_ID'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'CO_CONTRATOPRINCIPAL'
        Title.Alignment = taCenter
        Title.Caption = 'Contrato'
        Width = 68
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'EM_CUIT'
        Title.Alignment = taCenter
        Title.Caption = 'Cuit'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EM_NOMBRE'
        Title.Caption = 'R. Social'
        Width = 177
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'EB_PERIODOVIGENCIADESDE'
        Title.Alignment = taCenter
        Title.Caption = 'Per'#237'odo Desde'
        Width = 95
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'EB_PERIODOVIGENCIAHASTA'
        Title.Alignment = taCenter
        Title.Caption = 'Per'#237'odo Hasta'
        Width = 89
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EB_USUALTA'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'EB_FECHABAJA'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'EB_USUBAJA'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'EM_ID'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'DESCBONIF'
        Title.Caption = 'Tipo Bonificaci'#243'n'
        Width = 118
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESCAPLICA'
        Title.Caption = 'Aplica a'
        Width = 116
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EB_FIJOCUOTA'
        Title.Alignment = taCenter
        Title.Caption = 'Importe Fijo Cuota'
        Width = 86
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EB_FIJOFONDO'
        Title.Alignment = taCenter
        Title.Caption = 'Importe Fijo Fondo'
        Width = 82
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EB_BONIFICACIONANT'
        Title.Alignment = taCenter
        Title.Caption = 'Bonif. Previa'
        Width = 95
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'FECHAAUTORIZA'
        Title.Alignment = taCenter
        Title.Caption = 'F. Autorizaci'#243'n'
        Width = 81
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'USUAUTORIZA'
        Title.Alignment = taCenter
        Title.Caption = 'Usuario Autorizaci'#243'n'
        Width = 111
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EB_PORCENTAJE'
        Title.Alignment = taCenter
        Title.Caption = 'Porcentaje'
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MOTIVO'
        Title.Caption = 'Motivo'
        Width = 120
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'FECHAALTA'
        Title.Alignment = taCenter
        Title.Caption = 'F. Carga'
        Width = 76
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'regimen'
        Title.Caption = 'R'#233'gimen'
        Width = 150
        Visible = True
      end>
  end
  inherited fpAbm: TFormPanel
    Left = 184
    Top = 184
    Width = 582
    Height = 303
    Caption = 'Agregar empresas a bonificar'
    BorderStyle = bsSingle
    ExplicitLeft = 184
    ExplicitTop = 184
    ExplicitWidth = 582
    ExplicitHeight = 303
    DesignSize = (
      582
      303)
    inherited BevelAbm: TBevel
      Left = 0
      Top = 267
      Width = 574
      ExplicitLeft = 0
      ExplicitTop = 267
      ExplicitWidth = 574
    end
    object lblPerVigenciaDesde: TLabel [1]
      Left = 10
      Top = 138
      Width = 96
      Height = 13
      Caption = 'Per'#237'odo Vig. Desde:'
    end
    object Label5: TLabel [2]
      Left = 170
      Top = 138
      Width = 93
      Height = 13
      Caption = 'Per'#237'odo Vig. Hasta:'
    end
    object lblCuit: TLabel [3]
      Left = 10
      Top = 24
      Width = 18
      Height = 13
      Caption = 'Cuit'
    end
    object lblTipo: TLabel [4]
      Left = 10
      Top = 49
      Width = 21
      Height = 13
      Caption = 'Tipo'
    end
    object Label3: TLabel [5]
      Left = 285
      Top = 49
      Width = 41
      Height = 13
      Caption = 'Aplicar a'
    end
    object Label1: TLabel [6]
      Left = 10
      Top = 186
      Width = 71
      Height = 13
      Caption = 'Observaciones'
    end
    inherited btnAceptar: TButton
      Left = 424
      Top = 273
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
      ExplicitLeft = 424
      ExplicitTop = 273
    end
    inherited btnCancelar: TButton
      Left = 502
      Top = 273
      TabOrder = 9
      ExplicitLeft = 502
      ExplicitTop = 273
    end
    inline fraEmpresaABonificar: TfraEmpresa
      Left = 38
      Top = 20
      Width = 534
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      ExplicitLeft = 38
      ExplicitTop = 20
      ExplicitWidth = 534
      ExplicitHeight = 21
      DesignSize = (
        534
        21)
      inherited lbContrato: TLabel
        Left = 431
        ExplicitLeft = 431
      end
      inherited edContrato: TIntEdit
        Left = 477
        Width = 56
        ExplicitLeft = 477
        ExplicitWidth = 56
      end
      inherited cmbRSocial: TArtComboBox
        Top = 1
        Width = 295
        ExplicitTop = 1
        ExplicitWidth = 295
      end
    end
    object edPeriodoHasta: TPeriodoPicker
      Left = 267
      Top = 135
      Width = 49
      Height = 20
      TabOrder = 5
      Color = clWindow
      Periodo.AllowNull = True
      Periodo.Orden = poAnoMes
      Periodo.Separador = #0
      Periodo.Mes = 12
      Periodo.Ano = 2005
      Periodo.Valor = '200512'
      Periodo.MaxPeriodo = '205511'
      Periodo.MinPeriodo = '195512'
      WidthMes = 19
      Separation = 0
      ShowButton = False
      ShowOrder = poAnoMes
      ReadOnly = False
      Enabled = True
      Fuente.Charset = DEFAULT_CHARSET
      Fuente.Color = clWindowText
      Fuente.Height = -11
      Fuente.Name = 'MS Sans Serif'
      Fuente.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      NullDropDown = SetMax
    end
    object edPeriodoDesde: TPeriodoPicker
      Left = 111
      Top = 135
      Width = 49
      Height = 20
      TabOrder = 4
      Color = clWindow
      Periodo.AllowNull = True
      Periodo.Orden = poAnoMes
      Periodo.Separador = #0
      Periodo.Mes = 12
      Periodo.Ano = 2005
      Periodo.Valor = '200512'
      Periodo.MaxPeriodo = '205512'
      Periodo.MinPeriodo = '195512'
      WidthMes = 19
      Separation = 0
      ShowButton = False
      ShowOrder = poAnoMes
      ReadOnly = False
      Enabled = True
      Fuente.Charset = DEFAULT_CHARSET
      Fuente.Color = clWindowText
      Fuente.Height = -11
      Fuente.Name = 'MS Sans Serif'
      Fuente.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      NullDropDown = SetMax
    end
    object cbTipoBonificacion: TExComboBox
      Left = 38
      Top = 45
      Width = 237
      Height = 22
      Style = csOwnerDrawFixed
      TabOrder = 1
      OnChange = cbTipoBonificacionChange
      Items.Strings = (
        'F=Fija'
        'V=Variable'
        'P=Porcentual')
      ItemIndex = -1
      Options = [coGridLines]
      ValueWidth = 64
    end
    object cbAplicar: TExComboBox
      Left = 331
      Top = 45
      Width = 241
      Height = 22
      Style = csOwnerDrawFixed
      TabOrder = 2
      Text = 'C=Cuota Solamente'
      OnChange = cbAplicarChange
      Items.Strings = (
        'C=Cuota Solamente'
        'A=Cuota y Fondo')
      ItemIndex = 0
      Options = [coGridLines]
      ValueWidth = 64
    end
    object gbImporteFijoMensual: TGroupBox
      Left = 10
      Top = 71
      Width = 563
      Height = 57
      Caption = 'Importe Fijo Mensual'
      TabOrder = 3
      object lblCuota: TLabel
        Left = 18
        Top = 25
        Width = 47
        Height = 13
        Caption = 'Cuota Fijo'
      end
      object lblFondo: TLabel
        Left = 195
        Top = 25
        Width = 49
        Height = 13
        Caption = 'Fondo Fijo'
      end
      object lblPorcentaje: TLabel
        Left = 369
        Top = 25
        Width = 51
        Height = 13
        Caption = 'Porcentaje'
      end
      object edCuotaFijo: TCurrencyEdit
        Left = 70
        Top = 21
        Width = 98
        Height = 21
        Margins.Left = 1
        Margins.Top = 1
        AutoSize = False
        DisplayFormat = '$ ,0.00;$ - ,0.00'
        MaxLength = 12
        TabOrder = 0
      end
      object edFondoFijo: TCurrencyEdit
        Left = 248
        Top = 21
        Width = 98
        Height = 21
        Margins.Left = 1
        Margins.Top = 1
        AutoSize = False
        DisplayFormat = '$ ,0.00;$ - ,0.00'
        Enabled = False
        MaxLength = 12
        TabOrder = 1
      end
      object edPorcentaje: TCurrencyEdit
        Left = 425
        Top = 21
        Width = 98
        Height = 21
        Margins.Left = 1
        Margins.Top = 1
        AutoSize = False
        DisplayFormat = '0.00'
        Enabled = False
        MaxLength = 12
        TabOrder = 2
      end
    end
    object cbDebeExistirBonif: TCheckBox
      Left = 8
      Top = 163
      Width = 170
      Height = 17
      Alignment = taLeftJustify
      Caption = 'Debe existir bonificaci'#243'n previa'
      TabOrder = 6
    end
    object edObservaciones: TMemo
      Left = 38
      Top = 201
      Width = 530
      Height = 61
      MaxLength = 255
      TabOrder = 7
    end
  end
  object gbFiltro: TJvgGroupBox [4]
    Left = 0
    Top = 1
    Width = 792
    Height = 66
    Align = alTop
    Caption = 'Filtros'
    Color = clBtnFace
    ParentColor = False
    TabOrder = 4
    Border.Inner = bvNone
    Border.Outer = bvNone
    Border.Bold = False
    CaptionAlignment = fcaWidth
    CaptionBorder.Inner = bvRaised
    CaptionBorder.Outer = bvNone
    CaptionBorder.Bold = False
    CaptionGradient.FromColor = 10461087
    CaptionGradient.ToColor = clSilver
    CaptionGradient.Active = True
    CaptionGradient.Orientation = fgdHorizontal
    CaptionShift.X = 8
    CaptionShift.Y = 0
    Colors.Text = clHighlightText
    Colors.TextActive = clHighlightText
    Colors.Caption = clBtnShadow
    Colors.CaptionActive = clBtnShadow
    Colors.Client = clBtnFace
    Colors.ClientActive = clBtnFace
    Gradient.FromColor = clBlack
    Gradient.ToColor = clGray
    Gradient.Active = False
    Gradient.Orientation = fgdHorizontal
    Options = [fgoCanCollapse, fgoFilledCaption, fgoFluentlyCollapse, fgoFluentlyExpand, fgoHideChildrenWhenCollapsed, fgoSaveChildFocus]
    TransparentCaption = True
    GroupIndex = 1
    GlyphCollapsed.Data = {
      
      0F0F0F0F0F0F0F0F0F00}
    GlyphExpanded.Data = {
      
      0F0F0F0F0F0F0F0F0F00}
    DesignSize = (
      792
      66)
    FullHeight = 56
    object gbFechaAlta: TGroupBox
      Left = 520
      Top = 19
      Width = 267
      Height = 43
      Anchors = [akTop, akRight]
      Caption = 'Fecha de Carga'
      TabOrder = 1
      object Label2: TLabel
        Left = 8
        Top = 18
        Width = 31
        Height = 13
        Caption = 'Desde'
      end
      object Label4: TLabel
        Left = 138
        Top = 18
        Width = 28
        Height = 13
        Caption = 'Hasta'
      end
      object edFAltaDesdeFiltro: TDateComboBox
        Left = 44
        Top = 15
        Width = 88
        Height = 21
        MaxDateCombo = edFAltaHastaFiltro
        TabOrder = 0
      end
      object edFAltaHastaFiltro: TDateComboBox
        Left = 171
        Top = 15
        Width = 88
        Height = 21
        MinDateCombo = edFAltaDesdeFiltro
        TabOrder = 1
      end
    end
    object GroupBox1: TGroupBox
      Left = 5
      Top = 19
      Width = 513
      Height = 43
      Anchors = [akLeft, akTop, akRight]
      Caption = 'Empresa'
      TabOrder = 0
      DesignSize = (
        513
        43)
      object lblCuitFiltro: TLabel
        Left = 8
        Top = 18
        Width = 18
        Height = 13
        Caption = 'Cuit'
      end
      inline fraEmpresaFiltro: TfraEmpresa
        Left = 33
        Top = 15
        Width = 472
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        ExplicitLeft = 33
        ExplicitTop = 15
        ExplicitWidth = 472
        ExplicitHeight = 21
        DesignSize = (
          472
          21)
        inherited lbContrato: TLabel
          Left = 368
          ExplicitLeft = 368
        end
        inherited edContrato: TIntEdit
          Left = 415
          Width = 56
          ExplicitLeft = 415
          ExplicitWidth = 56
        end
        inherited cmbRSocial: TArtComboBox
          Top = 1
          Width = 233
          ExplicitTop = 1
          ExplicitWidth = 233
        end
      end
    end
  end
  inherited sdqConsulta: TSDQuery
    SQL.Strings = (
      
        'SELECT ieb.eb_id, aco.co_contrato, ieb.eb_periodovigenciahasta, ' +
        'ieb.eb_periodovigenciadesde, ieb.eb_fechaalta, ieb.eb_usualta,'
      
        '       ieb.eb_fechabaja, ieb.eb_usubaja, aco.co_idempresa, aem.e' +
        'm_id, aem.em_cuit, aem.em_nombre, eb_tipobonificacion,'
      
        '       DECODE(eb_tipobonificacion, '#39'F'#39', '#39'Fija'#39', '#39'V'#39', '#39'Variable'#39',' +
        ' '#39'P'#39', '#39'Porcentual'#39', '#39#39') descbonif, eb_aplica,'
      
        '       DECODE(eb_aplica, '#39'C'#39', '#39'Cuota Solamente'#39', '#39'A'#39', '#39'Cuota y F' +
        'ondo'#39', '#39#39') descaplica, eb_fijocuota, eb_fijofondo,'
      '       eb_bonificacionant'
      
        '  FROM afi.aem_empresa aem, afi.aco_contrato aco, emi.ieb_empres' +
        'abonificacion ieb'
      ' WHERE (aem.em_id = aco.co_idempresa)'
      '   AND (aem.em_id = ieb.eb_idempresa)'
      
        '   AND (co_contrato = art.afiliacion.get_ultcontrato(aem.em_cuit' +
        '))'
      '   AND 1= 2')
  end
  object ShortCutControl1: TShortCutControl
    ShortCuts = <
      item
        Key = 116
        LinkControl = tbRefrescar
      end
      item
        Key = 16462
        LinkControl = tbNuevo
      end
      item
        Key = 16461
        LinkControl = tbModificar
      end
      item
        Key = 16430
        LinkControl = tbEliminar
      end
      item
        Key = 16460
        LinkControl = tbLimpiar
      end
      item
        Key = 16463
        LinkControl = tbOrdenar
      end
      item
        Key = 16457
        LinkControl = tbImprimir
      end
      item
        Key = 16453
        LinkControl = tbExportar
      end
      item
        Key = 16467
        LinkControl = tbSalir
      end>
    Left = 80
    Top = 256
  end
end
