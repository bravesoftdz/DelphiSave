unit unConsultaJuicioParteDemandada;
...
uses
  ...
  artSeguridad, QueryPrint, QueryToFile, ExportDialog, SortDlg,
  DB, SDEngine, Grids, DBGrids, RXDBCtrl, ArtComboBox, ArtDBGrid, ComCtrls,
  ToolWin, StdCtrls, unArtDbFrame, unFraEmpresa, Mask, 
  DateComboBox, PatternEdit, unfraAbogadosLegales, unArtFrame,
  unArtDBAwareFrame, unFraCodigoDescripcion, IntEdit, ExtCtrls,
  unFraTrabajador, unfraCPR_PRESTADOR, unfraCtbTablas, Buttons,
  unABMSeguimientoJuiciosParteDemandada,
  unFraStaticCodigoDescripcion, unFraStaticCTB_TABLAS, FormPanel;

type
  TfrmConsultaJuicioParteDemandada = class(TfrmConsultaJuiciosEnTramite)
    chkNoSiniestro: TCheckBox;
    chkFederal: TCheckBox;
    chkGPBA: TCheckBox;
    fraMediador: TfraAbogadosLegales;
    lblMediador: TLabel;
    gbMediacion: TGroupBox;
    lblNroExpedienteMediacion: TLabel;
    lblOriganoMediacion: TLabel;
    edExpedienteMediacion: TPatternEdit;
    fraOrganismoMediacion: TfraCodigoDescripcion;
    lblNroOrdenRaj: TLabel;
    edNroOrdenRAJ: TPatternEdit;
    lblFechaIngresoRaj: TLabel;
    edFechaIngresoRaj: TDateComboBox;
    lblReservaCapital: TLabel;
    lblReservaHonorarios: TLabel;
    edReservaHonorariosDesde: TCurrencyEdit;
    edReservaCapitalDesde: TCurrencyEdit;
    lblReservaCapitalHasta: TLabel;
    lblReservaHonorariosHasta: TLabel;
    edReservaCapitalHasta: TCurrencyEdit;
    edReservaHonorariosHasta: TCurrencyEdit;
    chkSinReservasHonorarios: TCheckBox;
    chkSinReservaCapital: TCheckBox;
    ...
    tbCierredeReserva: TToolButton;
    tbPedidosDePaquetes: TToolButton;
    fpPaqueteResumenWeb: TFormPanel;
    Bevel1: TBevel;
    lbFechaDesde: TLabel;
    btnCopRecepAceptar: TButton;
    btnCopRecepCancelar: TButton;
    edFechaPaqueteDesde: TDateComboBox;
    edFechaPaqueteHasta: TDateComboBox;
    lbFechaHasta: TLabel;
    Panel1: TPanel;
    pnlBottom: TPanel;
    lbCantidad: TLabel;
    ...
    procedure tbNuevoClick(Sender: TObject);reintroduce;
    procedure tbCierredeReservaClick(Sender: TObject);
    procedure tbPedidosDePaquetesClick(Sender: TObject);
  ...
  private
    ...
    sSelect: String;
    sWhere: String;
    FVerEnvioWebEstudios: Boolean;
    procedure CalcularFiltro(var from, where: String);
    procedure CalcularSelect(var Select: String);
    procedure SetVerEnvioWebEstudios(const Value: Boolean);
  protected
    procedure ChangeReclamante(Sender: TObject); override;
    procedure DoCargaAdministracionJuicio(Sender: TObject);override;
  public
    property VerEnvioWebEstudios: Boolean read FVerEnvioWebEstudios write SetVerEnvioWebEstudios;
  end;

var
  frmConsultaJuicioParteDemandada: TfrmConsultaJuicioParteDemandada;

implementation

{$R *.dfm}
uses
  Strfuncs, AnsiSql, unSesion, unDmPrincipal, unConstLegales, General,
  unABMSeguimientoJuicios, unCustomConsulta,unCierredeReserva, CustomDlgs, SDCommon, unRptInformeGestion;
const
  OD_TRABAJADOR = 0;
...
{ TfrmConsultaJuicioParteDemandada }

procedure TfrmConsultaJuicioParteDemandada.FormCreate(Sender: TObject);
begin
  inherited;
  VerEnvioWebEstudios := False;

  fraEmpresaDemandante.ShowBajas := True;
{  with fraOrganismoMediacion do
  begin
    TableName   := 'legales.lom_organismomediacion';
    FieldID     := 'om_id';
    FieldCodigo := 'om_id';
    FieldDesc   := 'om_descripcion';
    FieldBaja   := 'om_fechabaja';
    Showbajas   := True;
  end;
 }
  ...
  fraReclamo.ExtraCondition := ' AND RC_ETAPA like ''%J%''';
  fraTipoDocumento.Clave := 'TDOC';
  if FNivelUsuario <> NIVELSUPERIOR then
  begin
    Grid.ColumnByField['jt_reservagestion'].Visible := false;
  end;
end;

procedure TfrmConsultaJuicioParteDemandada.CalcularFiltro(var from, where: String);
begin
////*********************** GENERAL ********************** //////
  ...
  if chkNoSiniestro.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.jt_juicionosiniestro = 1 ';

  if chkFederal.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.jt_federal = 1 ';

  if chkGPBA.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.jt_gpba = 1 ';

  if chkGPBAMalaPraxis.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_GPBAMALAPRAXIS = ''S'' ';

  if chkRebeldia.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.jt_rebeldia = 1 ';
  ...
  if fraMediador.IsSelected then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_IDMEDIADOR=' + fraMediador.Codigo;
...
{  if fraOrganismoMediacion.IsSelected then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_IDORGANISMOMEDIACION = ' + fraOrganismoMediacion.Codigo;
}
...
{  if not IsEmptyString(edExpedienteMediacion.Text) then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_EXPEDIENTEMEDIACION=' + SqlValue(edExpedienteMediacion.Text);
}
  if not IsEmptyString(edNroOrdenRAJ.Text) then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_NUMEROORDENRAJ =' + SqlValue(edNroOrdenRAJ.Text);

  if not edFechaIngresoRaj.IsEmpty then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.JT_FECHAINGRESORAJ =' + SqlDate(edFechaIngresoRaj.Date);
  ...
  if chkSinAbogadoDesignado.Checked then
    Where := Where + ' AND LJT_JUICIOENTRAMITE.jt_fechaasign IS NULL ' + #13#10
  else if not edFechaAsignDesde.IsEmpty then
  begin
  	if not edFechaAsignHasta.IsEmpty then
      Where := Where + SqlBetween( ' AND LJT_JUICIOENTRAMITE.jt_fechaasign ', edFechaAsignDesde.Date, edFechaAsignHasta.Date)
    else
      Where := Where + SqlBetween( ' AND LJT_JUICIOENTRAMITE.jt_fechaasign ', edFechaAsignDesde.Date, 0);
  end
  else if not edFechaAsignHasta.IsEmpty then
  	Where := Where + SqlBetween( ' AND LJT_JUICIOENTRAMITE.jt_fechaasign ', 0, edFechaAsignHasta.Date);
  ...
  Where := Where + SqlBetween(' AND LJT_JUICIOENTRAMITE.JT_IMPORTERESERVA ', edReservaCapitalDesde.Value, edReservaCapitalHasta.Value);
  Where := Where + SqlBetween(' AND LJT_JUICIOENTRAMITE.JT_IMPORTERESERVAHONORARIOS ', edReservaHonorariosDesde.Value, edReservaHonorariosHasta.Value);
...
{  if not IsEmptyString(edNombrePersona.Text) then
  	Where := Where + ' AND LOD.OD_NOMBRE LIKE ''%' + Trim(edNombrePersona.Text) + '%''';
}
{  if not IsEmptyString(edCuil.Text) then
  	Where := Where + ' AND LOD.OD_CUITCUIL = ' + SqlValue(edCuil.Text);
}
...
procedure TfrmConsultaJuicioParteDemandada.tbRefrescarClick(Sender: TObject);
begin
	sWhere  := '';
  sFrom   := '';
  sSelect := '';
  ...
  if chkARecuperar.Checked then
    sWhere := sWhere + ' AND JT_ARECUPERAR = ''S''';


  sdqConsulta.SQL.Text :=
  	'SELECT DISTINCT decode(JT_ESTADOMEDIACION, ''M'', ''Mediación'', ''JM'', ''Mediación-Juicio'', ''Juicio Parte Demandada'') MediacionJuicio,' +
                   ...
                   ' ljt_juicioentramite.jt_idestado, ljt_juicioentramite.jt_idabogado,ljt_juicioentramite.jt_reservagestion,' +
                   ...
                   ' ljt_juicioentramite.JT_NUMEROCARPETA,ljt_juicioentramite.jt_rebeldia, ' +
                   ' art.legales.get_importepagosjucio(jt_id) AS totalpagado '+sSelect +
...
procedure TfrmConsultaJuicioParteDemandada.tbLimpiarClick(Sender: TObject);
begin
  inherited;
  ...
  fraMediador.Codigo := '';
  //fraOrganismoMediacion.Codigo := '';
  //edExpedienteMediacion.Text := '';
  edNroOrdenRAJ.Text := '';
  edFechaIngresoRaj.Date := 0;
  edReservaCapitalDesde.Clear;
  edReservaCapitalHasta.Clear;
  edReservaHonorariosDesde.Clear;
  edReservaHonorariosHasta.Clear;
  edNroExpediente.Clear;
  edAnioExpediente.Clear;
  edExpedienteExport.Clear;
  chkSinReservaCapital.Enabled := False;
  chkSinReservasHonorarios.Enabled := False;
//  edNombrePersona.Text := '';
//  edCuil.Text := '';
  fraEmpresaDemandante.Clear;
end;

procedure TfrmConsultaJuicioParteDemandada.CalcularSelect(var Select: String);
var
  sWhere: String;
begin
...
{procedure TfrmConsultaJuicioParteDemandada.CalcularSumatoria;
begin
...
procedure TfrmConsultaJuicioParteDemandada.DoCargaAdministracionJuicio(Sender: TObject);
begin
...
(*
        GuardarInfoBloqueo(sdqConsulta.FieldByName('jt_id').AsInteger, 'J', Sesion.LoginName);
*)
        Modificacion(sdqModificacionRegistro);
(*
        BeginTrans(true);
        EliminarInfoBloqueo(dmPrincipal, sdqConsulta.FieldByName('jt_id').AsInteger, 'J', Sesion.LoginName);
        CommitTrans(true);
*)
      except
        ...
        begin
          ...
          begin
            Rollback(true);
            MessageDlg('El registro está siendo utilizado por el usuario ' + GetInfoBloqueo(sdqConsulta.FieldByName('jt_id').AsInteger, 'J') + '.' + #13#10 + 'Por esa razón no podrá guardar ningún cambio.', mtWarning, [mbOK], 0);
...
procedure TfrmConsultaJuicioParteDemandada.tbNuevoClick(Sender: TObject);
begin
  with TfrmAdminstracionJuicioParteDemandanda.Create(Self) do
  try
    if AltaJuicio then
...
procedure TfrmConsultaJuicioParteDemandada.tbCierredeReservaClick(Sender: TObject);
begin
  Abrir(TfrmCierredeReserva, frmCierredeReserva, tmvModal, nil);
end;

procedure TfrmConsultaJuicioParteDemandada.tbPedidosDePaquetesClick(Sender: TObject);
begin
  fpPaqueteResumenWeb.ShowModal;
end;

procedure TfrmConsultaJuicioParteDemandada.btnCopRecepAceptarClick(Sender: TObject);
var
  email : String;
  nropedido : Integer;
begin
  ...
  email := ValorSql(
    ...
    '  WHERE se_usuario = '+SqlValue(Sesion.UserID));

  nropedido := ValorSql(
    ' SELECT art.seq_hpr_id.NEXTVAL '+
    '   FROM DUAL');

  { Insertar pedido }

  EjecutarSqlST(
    ' INSERT INTO hys.hpr_pedidosreporte '+
    '             (pr_id, pr_emailrespuesta, pr_fechaalta, pr_usualta, '+
    '              pr_origenpedido, pr_asunto, pr_firma) '+
    '      VALUES ('+SqlValue(nropedido)+','+SqlValue(email) +', SYSDATE, ''SISTEMAS'', '+
    '              ''S'', ''Modificación Estudios Juridicos(WEB)'', '''')');

  { Insertar parámetros }
  EjecutarSqlST(
    ' INSERT INTO hys.hpp_parametrospedido '+
    '             (pp_id, pp_parametro, pp_valor, pp_idpedidoreporte) '+
    '      VALUES (art.seq_hpp_id.NEXTVAL, ''EMAILFILTRO'','+SqlValue(email)+','+
    SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpp_parametrospedido '+
    '             (pp_id, pp_parametro, pp_valor, pp_idpedidoreporte) '+
    '      VALUES (art.seq_hpp_id.NEXTVAL, ''FECHADESDEFILTRO'', '+
    SqlDate(edFechaPaqueteDesde.Date)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpp_parametrospedido '+
    '             (pp_id, pp_parametro, pp_valor, pp_idpedidoreporte) '+
    '      VALUES (art.seq_hpp_id.NEXTVAL, ''FECHAHASTAFILTRO'', '+
    SqlValue(edFechaPaqueteHasta.Date)+','+SqlValue(nropedido)+')');

  { Insertar informes}
  EjecutarSqlST(
    ' INSERT INTO hys.hpw_pedidosweb '+
    '             (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '      VALUES (art.seq_hpw_id.NEXTVAL, 197, SYSDATE,'+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpw_pedidosweb '+
    '            (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '     VALUES (art.seq_hpw_id.NEXTVAL, 198, SYSDATE,'+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpw_pedidosweb '+
    '             (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '      VALUES (art.seq_hpw_id.NEXTVAL, 199, SYSDATE,'+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    'INSERT INTO hys.hpw_pedidosweb '+
    '            (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '     VALUES (art.seq_hpw_id.NEXTVAL, 200, SYSDATE,'+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpw_pedidosweb '+
    '             (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '      VALUES (art.seq_hpw_id.NEXTVAL, 201, SYSDATE, '+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');

  EjecutarSqlST(
    ' INSERT INTO hys.hpw_pedidosweb '+
    '             (pw_id, pw_idreporte, pw_fechaalta, pw_usualta, pw_idpedido) '+
    '      VALUES (art.seq_hpw_id.NEXTVAL, 202, SYSDATE,'+
    SqlValue(Sesion.UserID)+','+SqlValue(nropedido)+')');
  CommitTrans();
  MessageDlg('En unos minutos recibira el mail con los datos de la carga web.', mtInformation, [mbOK], 0);
  fpPaqueteResumenWeb.Close;
end;

procedure TfrmConsultaJuicioParteDemandada.SetVerEnvioWebEstudios(const Value: Boolean);
begin
  FVerEnvioWebEstudios        := Value;
  tbPedidosDePaquetes.Visible := Value;
end;

procedure TfrmConsultaJuicioParteDemandada.tbSalirClick(Sender: TObject);
begin
  inherited;
  self.ModalResult:=2;
  self.close;
...
procedure TfrmConsultaJuicioParteDemandada.ChangeReclamante(Sender: TObject);
var
  Reclamante: Integer;
begin
  if fraReclamante.IsSelected then
