...
resourcestring
  LJT_GESTION_ALTA = 'Alta... (Administración de Mediaciones)';
  LJT_GESTION_MODIF = 'Modificación... (Administración de Mediaciones)';
...
begin
  ...
  try
    ...
    on E: Exception do
    	raise Exception.Create('Error al guardar la Mediación: ' + E.Message);
  end;
end;

function TfrmABMSeguimientoMediacion.Validar: boolean;
begin
  Verificar(edFechaInicio.Date = 0, edFechaInicio,'Debe Ingresar la Fecha de Notificación.' );
  Verificar(not fraTipoMediacion.IsSelected,fraTipoMediacion, 'Debe Seleccionar Tipo Mediación');
...
//  Verificar(not fraProceso.IsSelected, fraProceso,'Debe seleccionar el Proceso.' );
  ...
  Verificar(edFECHAASIGNADO.Date = 0, edFECHAASIGNADO,'Debe Ingresar la Fecha de Asignación.' );
  ...
  Verificar(not(fraNoSiniestro.IsSelected) and chkNoSiniestro.Checked,fraNoSiniestro,'Debe seleccionar una opción.');
...
begin
  nroFolio := ValorSql('SELECT art.legales.getproximanrofolio '+
           '  FROM dual');
   MessageDlg('Se generó el Nº de Folio ' + Inttostr(nroFolio), mtInformation, [mbOK], 0);
...
begin
  Verificar(edNroDocumentoOrigenDemanda.Text = '', edNroDocumentoOrigenDemanda, 'Debe existir el Número de Documento.');
...
begin
  ...
  begin
    ...
    Verificar( cbAplicacionImportes.ItemIndex = -1,cbAplicacionImportes,'Debe especificar concepto de aplicación.');
...
begin
  ...
  begin
    MessageBox(0, 'El bloqueo de pagos no es aplicable a esta mediación ya que no se '+#13+
...
begin
  ...
  begin
    ...
    Verificar(folio <> 0, edExpediente, 'Este expediente se repite en la mediación ' + IntToStr(folio) + '.');
  end;
end;
