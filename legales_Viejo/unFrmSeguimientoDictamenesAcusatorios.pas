...
begin
  ...
  begin
    if (MessageBox(Self.Handle, '¿Está seguro que desea cambiar el sumario a Sede Judicial?', 'Cambio a Sede Judicial', MB_ICONQUESTION or MB_YESNO or MB_TOPMOST or MB_DEFBUTTON1) = idYes) then
...
//                    'Tipo de Costas inválido. Existen Hon. a Abogados Oponentes cargados.',
  ...
  //                      edFechaInicio, edFechaInicio.Date <= DBDate, 'La fecha de fin debe ser anterior o igual a la actual',
                    ...
                    'Debe especificar un Número de DAC',
                    edFechaDac,
                    edFechaDac.Date > 0,
                    ...
                    'Solo puede especificar una  Fecha de Notificación DAC, si ha especificado una Fecha DAC',
                    edDisposicionSAL,
                    not IsEmptyString(edDisposicionSAL.Text),
                    'Debe especificar una Número de Disposición SAL',
                    edFechaSal,
                    edFechaSal.Date > 0,
                    'Debe especificar una Fecha para la Disposición SAL',
                    edFechaSal,
                    ...
                    'Debe especificar la fecha de fin sólo en el estado Terminado.',
                    edFechaFin,
                    not ((fraEstado.Value = ESTADO_TERMINADO) and (edFechaFin.Date = 0)),
  ...
  begin
    VerificarMultiple(['Guardando Dictamen Acusatorio',
                      ...
                      'Si Número de Resolución y Fecha Resolución debe especificarse conjuntamente',
                      ...
                      'Solo puede especificar la Fecha de Notificación de la Resolución si ha especificado el Número y Fecha de Resolución']);
  end else
  begin
    VerificarMultiple(['Guardando Dictamen Acusatorio',
                      edNroResolucion, (IsEmptyString(edNroResolucion.Text) and (edFechaResolucion.Date = 0) and (edFechaNotificacionResolucion.Date = 0)),
                      'No puede fijar nro de resolución y fecha resolución si no se especificó el tipo de verificación.'
                      ]);
  end;
  ...
  if not IsEmptyString(edNroResolucion.Text) and
    ...
    VerificarMultiple(['Guardando Dictamen Acusatorio',
                       edMultaSRT, edMultaSRT.Value > 0, 'Cuando especifica un Nro Resolución debe especificar monto de Multa'])
  else
    VerificarMultiple(['Guardando Dictamen Acusatorio',
...
//     sSqlTrans.Fields.AddDate('su_fechaaprobacionreservas', edFechaReserva.Date, true);
  ...
  {----}
    ...
    begin
      MsgBox('La Tasa de Justicia más los Honorarios del Abogado Oponente supera el 25% de la Multa SRT.' + #13+ #10 +
      'Ver Artículo 5 del Código Civil.', MB_ICONINFORMATION, 'Atención')
    end;
  ...
  if FModo = meAlta then
    MessageBox(Self.Handle,PChar('Se generó el Sumario Nro. ' + InttoStr(NroSumario)), 'Sumarios', MB_ICONINFORMATION or MB_OK);
  ModalResult := mrOk;
end;
{--------------------------------------------------------------------------------}
  ...
  begin
    ...
    begin
        try
          ...
          on E : Exception do begin
            MsgBox('No se pudo establecer conexión a la ' +
...
//      edFechaReserva.Date := DBDate;
  ...
  end else InvalidHint(edReservaCapital,'Las reservas ya se encuentran aprobadas.', 'Aprobación de Reservas');
end;
...
Autorización
  ...
  if (not sdqPagoSumario.IsEmpty) and
     (MessageDlg('¿Confirma la Aprobación del Pago Seleccionado?',
      mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
  begin
