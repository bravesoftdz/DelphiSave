...
resourcestring
  LJT_GESTION_ALTA = 'Alta... (Administración de Juicios)';
  LJT_GESTION_MODIF = 'Modificación... (Administración de Juicios)';
  LJT_GESTION_ALTAMEDIACION = 'Alta... (Administración de Mediaciones)';
  LJT_GESTION_MODIFMEDIACION = 'Modificación... (Administración de Mediaciones)';
  LJT_GESTION_CONSJUICIO = 'Consulta... (Administración de Juicios)';
  LJT_GESTION_CONSMEDIACION = 'Consulta... (Administración de Mediaciones)';
    ...
    try
      ...
      begin
        fraDatosJuzgado.gbJuzgados.Caption := fraDatosJuzgado.gbJuzgados.Caption + ' ~ ' +
                                              'Jurizdicción: ' + Q.FieldByName('jurisdiccion').AsString + ' - ' +
...
begin
  If FMediacion then
    If (MessageBox(0, '¿Confirma la elevación a juicio de la mediación?', 'Elevación A Juicio', MB_ICONQUESTION or MB_YESNO or MB_DEFBUTTON1) in [idYes, idNo]) then
...
begin
  ...
  begin
    MsgBox('El origen de demanda está dado de baja.' + #13 +
...
begin
  ...
  if not sdqOrigenDemanda.FieldByName('od_fechabaja').IsNull then
  begin
    MsgBox('El origen de demanda ya está dado de baja.', MB_ICONERROR + MB_OK);
    Exit
  end;
...
begin
  ...
  if not sdqEventoJuicioEnTramite.FieldByName('et_fechabaja').IsNull then
  begin
    MsgBox('El evento ya está dado de baja.', MB_ICONERROR + MB_OK);
    Exit
  end;
...
begin
  ...
  begin
    MsgBox('La pericia está dada de baja.' + #13 +
           'Sus datos no pueden ser modificados.', MB_ICONERROR + MB_OK);
    Exit
  end;
...
begin
  ...
  if not sdqPericiasJuicios.FieldByName('pj_fechabaja').IsNull then
  begin
    MsgBox('La pericia ya está dada de baja.', MB_ICONERROR + MB_OK);
    Exit
  end;
...
begin
  ...
  begin
    VerificarMultiple(['Guardando Pericia..',
                      ...
                      'Debe indicar un Fecha de Notificación de la Pericia',
                      dcbFechaNotificacionPerito,
                      dcbFechaNotificacionPerito.date >= edFECHANOTIFICACION.Date,
                      'La fecha de la pericia no puede ser anterior a la fecha de notificación del juicio.',
                      dcbFechaNotificacionPerito,
                      ...
                      'Debe completar la finalización de la pericia correctamente, completando fecha de pericia y resultado de pericia.']);
    Verificar(edFVencImpug.Date = 0, edFVencImpug,'Debe ingresa la fecha de vencimiento de impugnación.');
    Verificar(rgImpugnacion.ItemIndex = -1, rgImpugnacion,'Debe indicar la impugnación');
  end;
...
begin
  ...
  else InvalidMsg(dbgPagosEfectuados, 'No se puede cancelar un pago en este estado.', 'Cancelación de Pago');
...
begin
  ...
  if (sdqPagoLegal.FieldByName('pl_duplicado').AsString = 'S') and
     sdqPagoLegal.FieldByName('pl_estadoduplicidad').IsNull then
  begin
    MsgBox('El pago está pendiente de aprobacion por duplicado.' + #13 +
           'No puede imprimirse su liquidación.', MB_ICONERROR + MB_OK);
    Exit;
  end;

  if (sdqPagoLegal.FieldByName('pl_estadoduplicidad').AsString = 'R') then
  begin
    MsgBox('El pago está Rechazado.' + #13 +
           'No puede imprimirse su liquidación.', MB_ICONERROR + MB_OK);
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      if (MessageDlg('Con esta operación se obliga a reaprobar la reserva para este juicio.'+ #13+ #10+
...
begin
  ...
  begin
    InvalidMsg(fraEstado.cmbDescripcion, 'El estado del juicio es terminado, no pueden modificarse las reservas', 'Modificación Reservas', MB_ICONINFORMATION);
    Exit;
  end;
...
(*
  if not FReservasAprobadas then
  begin
   if (MessageDlg('¿Confirma la aprobación de las reservas?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
   begin
      EjecutarSqlST('UPDATE legales.ljt_juicioentramite ' +
  ...
  end else InvalidHint(ceReservaCapital,'Las reservas ya se encuentran aprobadas.', 'Aprobación de Reservas');
*)
end;
...
begin
  ...
  VerificarMultiple(['Guardando Acceso',
                     ...
                     'Debe especificar un usuario del área LEGALES.',
                     fraNivelesAcceso,
                     fraNivelesAcceso.Codigo <> '',
...
begin
{
  ...
  begin
    InvalidMsg(dbgOrigenDemanda, 'Debe agregar un evento con la fecha de notificación del tipo contestación de la demanda.', 'Guardar Juicio');
    abort;
  end;
}
...
begin
  ...
  begin
		Verificar((not sdqEmbargos.Active) or (sdqEmbargos.IsEmpty), dbgEmbargos, 'No ha seleccionado ningún registro.');
