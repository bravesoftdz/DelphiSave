...
var
  ...
  sLeyendaRetencion, sRetencion, sTextoExtra,
  sConcepto, sCaracter, sDiferencia, sNroExpediente: String;
begin
  with Liquidacion do
  begin
    ...
    sNroExpediente := lpad(Expediente.Siniestro, '0', 8) + lpad(Expediente.Orden, '0', 2) +            // TK 61021
                      lpad(Expediente.Recaida, '0', 2) + lpad(NroLiquidacion, '0', 3);

    qrlCodigoBarras.Caption  := '*' + sNroExpediente + '*';     // TK 61021
    qrlNroExpediente.Caption := sNroExpediente;                        // TK 61021
    ...
    qrlImpILTNoRem.Caption  := Get_AjusteDecimales(FloatToStr(ImporteLiquidNoRem), '$');
    ...
    qrlImpuesto.Enabled   := (Tipo = 'D');
    qrshImpuesto.Enabled  := (Tipo = 'D');

    if (Tipo <> 'D') and ((Diferencia >= 10) or (DiferenciaNoRem >= 10)) then
    begin
      sDiferencia := 'Asimismo les informamos que de la liquidación practicada surge una diferencia ';
      sDiferencia := sDiferencia + IIF(Diferencia >= 10, 'remunerativa de $' + Get_AjusteDecimales(FloatToStr(Diferencia)), '');
      sDiferencia := sDiferencia + IIF((Diferencia >= 10) and (DiferenciaNoRem >= 10), ' y ', '');
      sDiferencia := sDiferencia + IIF(DiferenciaNoRem >= 10, 'no remunerativa de $' + Get_AjusteDecimales(FloatToStr(DiferenciaNoRem)), '');
      sDiferencia := sDiferencia + ' a favor del accidentado, la cual deberán liquidar aplicándole las retenciones de Ley y enviarnos copia de la misma para su reintegro.';
    end
    else
      sDiferencia := '';

    qrlLeyenda.Caption := sDiferencia;
