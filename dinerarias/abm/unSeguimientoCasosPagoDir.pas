unit unSeguimientoCasosPagoDir;
...
type
  TfrmSeguimCasosPagoDir = class(TfrmCustomGridABM)
    GroupBox6: TGroupBox;
    Label7: TLabel;
...
var
  frmSeguimCasosPagoDir: TfrmSeguimCasosPagoDir; iIdLiquidadorLogueado: integer;
...
procedure TfrmSeguimCasosPagoDir.FormCreate(Sender: TObject);
var ssql: string;
begin
  inherited;
  ...
  begin
    ...
    ExtraCondition := ' AND MI_IDTIPOCASO = 1 ';
  end;
  ...
  begin
    ...
    ssql := '  SELECT   se_id ' +
              ...
              ' WHERE   lc_fechabaja IS NULL AND lc_usuario = :usu ' +
                ' AND   lc_idtipocaso = 1 AND se_usuario = lc_usuario ';
  ...
  begin
    ...
    ExtraCondition := ' AND ML_IDTIPOCASO = 1 ';
  end;
  ...
  fraFirmantePedidoDoc.Modulo := '83';
  ...
  tbViajar.Left := 330;
  tbViajar.Enabled := frmPrincipal.mnuLiqIncapacidades.Enabled;
...
procedure TfrmSeguimCasosPagoDir.fraMotivoIngresoABMEnter(Sender: TObject);
begin
  if (not SinEditABM.IsEmpty) then
    fraLiquidadorABM.Value := ValorSqlEx('SELECT se_id FROM art.use_usuarios, din.dlc_liquidadorescasos ' +
                                         ...
                                         '   AND lc_id = art.dinerarias.get_idliquidador(art.siniestro.get_idexpediente(:sin, :ord, :rec), :idmotivo) ',
                                                 [SinEditABM.Siniestro, SinEditABM.Orden, SinEditABM.Recaida, 1]);

end;

procedure TfrmSeguimCasosPagoDir.GridGetCellParams(Sender: TObject;
...
procedure TfrmSeguimCasosPagoDir.SinEditABMExit(Sender: TObject);
var ssql: string;
begin
...
procedure TfrmSeguimCasosPagoDir.tbAceptarClick(Sender: TObject);
var ssql: String;
begin
  Verificar((sdqConsulta.FieldByName('lc_usuario').AsString <> Sesion.UserID) and
            (not Seguridad.Claves.IsActive('Supervisor')), tbAceptar, 'No posee perfil para realizar esta acción');
  Verificar((sdqConsulta.FieldByName('el_codigo').AsString <> 'AS') and (sdqConsulta.FieldByName('el_codigo').AsString <> 'SV'),
            tbAceptar, 'Los casos a aceptar deben estar en estado ''Asignado'' o ''Asignado vencido''');
...
procedure TfrmSeguimCasosPagoDir.tbCasosPorLiqClick(Sender: TObject);
...
procedure TfrmSeguimCasosPagoDir.tbEliminarClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbHistoricoEstadosClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbLimpiarClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSeguimCasosPagoDir.tbModificarClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbMostrarOcultarColumnasClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbReasignarCasoClick(Sender: TObject);
begin
  fraLiquidadorReasignarCaso.Value := sdqConsulta.FieldByName('se_id').AsInteger;
  fpReasignarCaso.ShowModal;
end;

procedure TfrmSeguimCasosPagoDir.tbRefrescarClick(Sender: TObject);
var ssql: String;
begin
  ssql :=
          ...
          '       emi.itm_topemasas, use_usuarios ' +
...
procedure TfrmSeguimCasosPagoDir.tbSalir2Click(Sender: TObject);
begin
  Close;

end;

procedure TfrmSeguimCasosPagoDir.tbSalirClick(Sender: TObject);
begin
//  Close;

end;

procedure TfrmSeguimCasosPagoDir.tbSolicDocumClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbVerCasosRelacionadosClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbVerListPedidosDocClick(Sender: TObject);
begin
...
procedure TfrmSeguimCasosPagoDir.tbViajarClick(Sender: TObject);
begin
...
function TfrmSeguimCasosPagoDir.Validar : Boolean;
begin
  //Validar := False;
...
procedure TfrmSeguimCasosPagoDir.btnAceptarBajaClick(Sender: TObject);
var ssql: String;
begin
  ...
  begin
    BeginTrans;
    try
      ssql := 'UPDATE din.dcl_casosparaliquidar SET cl_idestado = 10, /*estado inactivo*/ ' +
                    ' cl_idmotivobaja = :idmotivo, cl_usubaja = :usuario, cl_fechabaja = SYSDATE ' +
              ' WHERE cl_id = :idcaso';

      EjecutarSqlSTEx(ssql, [fraMotivoBaja.ID, Sesion.UserID,sdqConsulta.FieldByName('cl_id').AsString]);
...
procedure TfrmSeguimCasosPagoDir.btnAceptarClick(Sender: TObject);
var ssql: String;
begin
  if Validar then
  begin
    ...
    try
      ssql := 'INSERT INTO din.dcl_casosparaliquidar (cl_idtipocaso, cl_idexpediente, cl_idestado, cl_idmotivoingreso, ' +
                   ...
                   'VALUES (1, art.siniestro.get_idexpediente(:sin, :ord, :rec), 2, :idmotivo, :idliquidador, :usuario, SYSDATE, ''S'') ';

      EjecutarSqlSTEx(ssql, [SinEditABM.Siniestro, SinEditABM.Orden, SinEditABM.Recaida, fraMotivoIngresoABM.ID,
...
procedure TfrmSeguimCasosPagoDir.btnAceptarModifClick(Sender: TObject);
var ssql: String;
begin
...
procedure TfrmSeguimCasosPagoDir.btnAceptarPedidoDocClick(Sender: TObject);
      ...
      begin
        ...
        idCarta := IntToStr(DoInsertarCarta(StrToInt(fraCDPedidoDoc.ID), SinEditPedidoDoc.Siniestro, SinEditPedidoDoc.Orden,
                                   ...
                                   ValorSql('SELECT at_id FROM cat_areatexto' +
                                              ' WHERE at_area = ''80'' AND at_modulo = ''83''' +
                                              '   AND at_fechabaja IS NULL')));
...
procedure TfrmSeguimCasosPagoDir.btnAceptarReasigCasoClick(Sender: TObject);
var ssql: string;
begin
...
function TfrmSeguimCasosPagoDir.ValidarPedidoDoc: boolean;
begin
  //ValidarPedidoDoc := False;
...
procedure TfrmSeguimCasosPagoDir.fraCDPedidoDocChange(Sender: TObject);
var ssql: String;
begin
  ...
  begin
    ...
    ssql := 'SELECT fi_id' +
            ...
            '   AND at_area = ''80'' AND at_modulo = ''83''' +
...
procedure TfrmSeguimCasosPagoDir.ClearControls;
begin
  SinEditABM.Clear;
