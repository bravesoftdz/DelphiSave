inherited frmLiquidacionesNoExigibles: TfrmLiquidacionesNoExigibles
  Left = 109
  Top = 125
  Width = 550
  Height = 400
  Caption = 'Liquidaciones No Exigibles'
  ...
  inherited sdqConsulta: TSDQuery
    SQL.Strings = (
        ...
        '         ROUND(SUM(liquidacion), 2) liquidaciones, ROUND(SUM(obr' +
        'asocial), 2) retos, ROUND(SUM(ingbrutos),2) retib,'
      
        '         ROUND(SUM(ganancias), 2) retgcias, ROUND(SUM(retiva), 2' +
        ') retiva, ROUND(SUM(factura), 2) facturas,'
        ...
        '          ROUND(SUM(retiva), 2) + ROUND(SUM(factura), 2)) saldof' +
        'inal'
      
        '    FROM (SELECT en_cuit cuit, en_nombre nombre, lc_comision + l' +
        'c_iva liquidacion, lc_obrasocial obrasocial,'
      
        '                 lc_ingbrutos ingbrutos, lc_ganancias ganancias,' +
        ' lc_retiva retiva, 0 factura'
      '            FROM xen_entidad, xlc_liqcomision'
      '           WHERE en_id = lc_identidad'
      '             AND lc_comision <> 0'
      '          UNION ALL'
      
        '          SELECT ve_cuit cuit, ve_nombre nombre, lc_comision + l' +
        'c_iva liquidacion, lc_obrasocial obrasocial,'
      
        '                 lc_ingbrutos ingbrutos, lc_ganancias ganancias,' +
        ' lc_retiva retiva, 0 factura'
      
        '            FROM xen_entidad, xve_vendedor, xev_entidadvendedor,' +
        ' xlc_liqcomision'
      ...
      '          UNION ALL'
        ...
        '          SELECT en_cuit cuit, en_nombre nombre, -(lc_comision +' +
        ' lc_iva) liquidacion, -lc_obrasocial obrasocial,'
      
        '                 -lc_ingbrutos ingbrutos, -lc_ganancias ganancia' +
        's, -lc_retiva retiva, 0 factura'
      '            FROM xen_entidad, xlc_liqcomision'
      '           WHERE en_id = lc_identidad'
      ...
      '          UNION ALL'
        ...
        '          SELECT ve_cuit cuit, ve_nombre nombre, -(lc_comision +' +
        ' lc_iva) liquidacion, -lc_obrasocial obrasocial,'
      
        '                 -lc_ingbrutos ingbrutos, -lc_ganancias ganancia' +
        's, -lc_retiva retiva, 0 factura'
      
        '            FROM xve_vendedor, xev_entidadvendedor, xlc_liqcomis' +
        'ion'
      '           WHERE ev_id = lc_identidadvendedor'
      '             AND ve_id = ev_idvendedor'
      '             AND lc_estado = '#39'C'#39
