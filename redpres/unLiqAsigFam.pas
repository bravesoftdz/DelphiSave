...
begin
  ...
  begin
    sSql := 'SELECT chl.hl_fechaingreso ' +
             ' FROM chl_historicolaboral chl, aco_contrato aco, ' +
                  ' aem_empresa aem, ctj_trabajador ' +
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            try
              LocalidadBusqueda := SqlResults[0];
              Secuencia         := SqlResults[1];
            finally
              SqlResults.Free;
            end;
            bHabLocalidad     := False;
...
begin
  ...
  sSql := 'SELECT mf_hijo, mf_hijodiscapacitado, mf_ayudaescolar ' +
           ' FROM smf_montosasigfamiliares ' +
          ' WHERE mf_codmonto = ' + SqlInt(CodMontoAsign);
  SqlResults := ValoresColSQL(sSql);
  try
    MontoHijo         := Nvl(SqlResults[0], 0);
    MontoHijoDiscap   := Nvl(SqlResults[1], 0);
    MontoAyudaEscolar := Nvl(SqlResults[2], 0);
...
begin
  ...
  sSql := 'SELECT ub_cpostal, ub_cpostala, ub_localidad, ub_provincia, ' +
          ...
          ' WHERE ub_siniestro = ' + SqlInt(Siniestro) +
            ' AND ub_orden = ' + SqlInt(Orden) +
            ' AND ub_recaida = ' + SqlInt(Recaida) +
            ' AND ub_provincia = pv_codigo(+) ' +
            ' AND ub_secuencia_partidoloc = co_secuencia(+) ';
  sqlResults := ValoresColSQL(sSql);
  try
    CPostalUbic               := Nvl(sqlResults[0], '');
    CPostalAUbic              := Nvl(sqlResults[1], '');
    LocalidadUbic             := Nvl(sqlResults[2], '');
    ProvinciaUbic             := Nvl(sqlResults[3], '');
    ProvinciaDescrUbic        := Nvl(sqlResults[4], '');
    ccoLocalidad              := Nvl(sqlResults[5], '');
    ccoPartido                := Nvl(sqlResults[6], '');
    ccoSecuencia_PartidoLoc   := Nvl(sqlResults[7], '');
    Result := (Nvl(sqlResults[8], '') <> '');
  finally
    sqlResults.Free;
  end;
end;
