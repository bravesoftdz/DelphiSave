...
type
  TfrmManTiposTraslado = class(TForm)
    sdqDatos: TSDQuery;
    dsDatos: TDataSource;
...
var
  frmManTiposTraslado: TfrmManTiposTraslado;
...
{--------------------------------------------------------------------------------------------------------------------------------}
procedure TfrmManTiposTraslado.Execute;
begin
     sdqDatos.Open;
...
{--------------------------------------------------------------------------------------------------------------------------------}
procedure TfrmManTiposTraslado.ABMDialogBeforeABM(Sender: TObject;ABMAction: TABMActions; var Cancel: Boolean);
var Sql:String;
begin
    ...
    if (ABMAction = aaModify) and (sdqDatosTB_BAJA.AsString = 'S' ) Then begin
       ...
       Cancel := True;
       If (sdqDatosTB_BAJA.AsString <> 'S') and ( MsgBox('¿Desea eliminar el Tipo de Traslado ''' + sdqDatosTB_CODIGO.Text + ''' ?', MB_ICONQUESTION + MB_YESNO) = IDYES ) Then begin
            Sql := ' UPDATE CTB_TABLAS ' +
                      ' SET TB_BAJA = ''S'' ' +
                    ' WHERE TB_CLAVE = ''TRAST'' ' +
...
{--------------------------------------------------------------------------------------------------------------------------------}
procedure TfrmManTiposTraslado.ABMDialogBeforePrint(Sender: TObject;var Cancel: Boolean);
begin
...
{--------------------------------------------------------------------------------------------------------------------------------}
procedure TfrmManTiposTraslado.ABMDialogGetCellParams(Sender: TObject;Field: TField; AFont: TFont; var Background: TColor; Highlight: Boolean);
begin
