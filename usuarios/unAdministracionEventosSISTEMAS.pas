unit unAdministracionEventosSISTEMAS;
...
uses
  ...
  unAdministracionEventos, FieldHider, ShortCutControl, artSeguridad, QueryPrint, QueryToFile,
  ExportDialog, SortDlg, Db, SDEngine, StdCtrls, ExtCtrls, FormPanel, Grids, DBGrids, RXDBCtrl,
  ArtComboBox, ArtDBGrid, ComCtrls, ToolWin, Mask, DateComboBox,
...
type
  TfrmAdministracionEventosSISTEMAS = class(TfrmAdministracionEventos)
    Label32: TLabel;
    ...
    ShortCutControlHijo1: TShortCutControl;
...
var
  frmAdministracionEventosSISTEMAS: TfrmAdministracionEventosSISTEMAS;
...
{$R *.DFM}
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.ClearControls;
begin
  inherited;
  ...
  fraSE_SECTORINVOLUCRADO_2PropiedadesChange(nil);
  if frafpSE_USUARIODERIVACIONTELEF.IsEmpty then
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.LoadControls;
begin
  sbScroll.VertScrollBar.Position := 0;
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.DoABMHijos;
begin
  inherited;
...
{------------------------------------------------------------------------------}
function TfrmAdministracionEventosSISTEMAS.DoABM: Boolean;
begin
  Result := inherited DoABM;
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.gbFiltrosEventosAdicionalesClick(Sender: TObject);
begin
  inherited;
  DoOcultarControles;   // si no hago esto, cuando colapso y vuelvo a expandir me vuelve a poner visible el frame de ingreso
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.FormCreate(Sender: TObject);
begin
  inherited;
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.DoOcultarControles;
begin
  fraSE_IDCODIGOINGRESO.Visible := False;
  Labelx2.Visible               := False;
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.tbLimpiarClick(Sender: TObject);
begin
  inherited;
  fraSE_IDRESPUESTA.Clear;
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.fraSE_IDRESPUESTA_2cmbDescripcionDropDown(Sender: TObject);
begin
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.fraSE_IDRESPUESTA_3cmbDescripcionDropDown(Sender: TObject);
begin
...
procedure DoMostrarEventosAgenda(AContrato: Integer; ACargaDesde: TDateTime = 0; ASoloMiSector: boolean = False);
begin
  with TfrmAdministracionEventosSISTEMAS.Create(nil) do
    try
...
{------------------------------------------------------------------------------}
function TfrmAdministracionEventosSISTEMAS.Validar: Boolean;
begin
  inherited Validar;
  ...
  begin
    ...
    Verificar(bCerrando and (ModoABM = maModificacion) and (Trim(edSE_OBSERVACIONREALIZADO_2.Lines.Text) = ''), edSE_OBSERVACIONREALIZADO_2, 'Debe indicar la respuesta.');
  end else
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.DoArmarSqlBusquedaAdicional;
begin
  inherited;

  SqlAddJoin(sSQL, 'ACO_CONTRATO CON', 'SE.SE_CONTRATO',    'CON.CO_CONTRATO', jtOuter);
  SqlAddJoin(sSQL, 'AEM_EMPRESA EMP',  'CON.CO_IDEMPRESA',  'EMP.EM_ID', jtOuter);
  SqlAddJoin(sSQL, 'CRR_RESPUESTA RR', 'SE.SE_IDRESPUESTA', 'RR.RR_ID', jtOuter);

  AddField(sSQL, 'EMP.EM_NOMBRE',                      'EMPRESA');
  AddField(sSQL, 'ART.UTILES.ARMAR_CUIT(EMP.EM_CUIT)', 'CUITEMPRESA');
  AddField(sSQL, 'CON.CO_CONTRATO',                    'CONTRATO');
  AddField(sSQL, 'RR.RR_DESCRIPCION',                  'DESCRRESPUESTA');

  if fraSE_IDRESPUESTA.IsSelected then
    AddCondition(sSQL, ' SE.SE_IDRESPUESTA = ' + SQLValue(fraSE_IDRESPUESTA.Value));
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.fraSE_IDRESPUESTA_2PropiedadesChange(Sender: TObject);
begin
  inherited;
  edSE_OBSERVACIONREALIZADO_2.Lines.Add(fraSE_IDRESPUESTA_2.Descripcion);
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.fraSE_IDRESPUESTA_3PropiedadesChange(Sender: TObject);
begin
  inherited;
  edSE_OBSERVACIONREALIZADO.Lines.Add(fraSE_IDRESPUESTA_3.Descripcion);
end;
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.btnAceptarSeleccionClick(Sender: TObject);
begin
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.Filtros(AValue: boolean);
begin
  inherited;
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.btnCerrarClick(Sender: TObject);
begin
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.fraSE_IDMOTIVOEVENTO_ABAJOcmbDescripcionDropDown(Sender: TObject);
begin
...
{------------------------------------------------------------------------------}
procedure TfrmAdministracionEventosSISTEMAS.frafpSE_IDCODIGOINGRESOPropiedadesChange(Sender: TObject);
begin
  inherited;
end;
{------------------------------------------------------------------------------}
