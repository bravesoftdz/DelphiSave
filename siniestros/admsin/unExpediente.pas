...
begin
  inherited;
  //Solo busco los datos si está tildado el check, sinó no (aunque puedan existir en la tabla)
...
begin
  ...
  Verificar((FechaRecepcion = 0) and (cmbEX_FECHARECEPCION.Date > FechaAlta), cmbEX_FECHARECEPCION, Format('La fecha de recepción no puede ser posterior a la fecha de carga del siniestro %s.', [DateToStr(FechaAlta)]));    // TK 22620

  if Recaida > 0 then
  begin
    FFechaAccidenteRecaida := edEX_FECHARECAIDA.Date;
    Verificar(edEX_FECHARECAIDA.Date = 0, edEX_FECHARECAIDA, 'Si se trata de una recaída, debe indicar la fecha de ocurrencia de la misma.');
    Verificar(edEX_FECHARECAIDA.Date > dHoy, edEX_FECHARECAIDA, Format('La fecha de recaída no puede ser posterior a hoy %s.', [DateToStr(dHoy)]));
    Verificar(edEX_FECHARECAIDA.Date <= edEX_FECHAACCIDENTE.Date, edEX_FECHARECAIDA, Format('La fecha de recaída no puede ser anterior o igual a la fecha del accidente %s.', [DateToStr(edEX_FECHAACCIDENTE.Date)]));
    ...
    Verificar(not(chkEX_INTERCURRENCIA.Checked) and (dFechaAux > 0) and
              (dFechaAux > edEX_FECHARECAIDA.Date), edEX_FECHARECAIDA, Format('La máxima fecha de egreso %s para recaídas anteriores a la actual es posterior a la fecha de recaída que se está ingresando.', [DateToStr(dFechaAux)]));
  end else
  begin
    FFechaAccidenteRecaida := edEX_FECHAACCIDENTE.Date;
  end;

  Verificar((FechaBajaMedica > 0) and (edEX_FECHAACCIDENTE.Date > FechaBajaMedica), edEX_FECHAACCIDENTE, Format('La fecha del accidente no puede ser posterior a la de baja médica %s.', [DateToStr(FechaBajaMedica)]));
  Verificar((FechaBajaMedica > 0) and (edEX_FECHARECAIDA.Date > FechaBajaMedica), edEX_FECHARECAIDA, Format('La fecha del recaída no puede ser posterior a la de baja médica %s.', [DateToStr(FechaBajaMedica)]));
  Verificar((FechaAltaMedica > 0) and (edEX_FECHAACCIDENTE.Date > FechaAltaMedica), edEX_FECHAACCIDENTE, Format('La fecha del accidente no puede ser posterior a la de alta médica %s.', [DateToStr(FechaAltaMedica)]));
  Verificar((FechaAltaMedica > 0) and (edEX_FECHARECAIDA.Date > FechaAltaMedica), edEX_FECHARECAIDA, Format('La fecha del recaída no puede ser posterior a la de alta médica %s.', [DateToStr(FechaAltaMedica)]));
  Verificar(fraEX_TIPO.IsEmpty, fraEX_TIPO.edCodigo, Msg_70);

  if fraEX_TIPO.EsEnfermedadProfesional then
  begin
    Verificar(edEX_FECHAMANIFESTACION.Date = 0, edEX_FECHAMANIFESTACION, 'Si se trata de una enfermedad profesional, debe indicar la fecha de primera manifestación.');
    Verificar(edEX_FECHAMANIFESTACION.Date > dHoy, edEX_FECHAMANIFESTACION, Format('La fecha de primera manifestación no puede ser posterior a hoy %s.', [DateToStr(dHoy)]));
    Verificar(edEX_FECHAMANIFESTACION.Date > FechaAccidenteRecaida, edEX_FECHAMANIFESTACION, Format('La fecha de primera manifestación no puede ser posterior a la del accidente/recaída %s.', [DateToStr(FechaAccidenteRecaida)]));
    Verificar(edEX_FECHAMANIFESTACION.Date < SIN_ART_MINFECHA, edEX_FECHAMANIFESTACION, Format('La fecha de primera manifestación no puede ser anterior a la del inicio de actividades de la ART %s.', [DateToStr(SIN_ART_MINFECHA)]));
    Verificar(edEX_FECHAEXPUESTO.Date = 0, edEX_FECHAEXPUESTO, 'Si se trata de una enfermedad profesional, debe indicar la fecha de exposición.');
    Verificar(edEX_FECHAEXPUESTO.Date > edEX_FECHAACCIDENTE.Date, edEX_FECHAEXPUESTO, Format('La fecha de exposición no puede ser posterior a la fecha del accidente %s.', [DateToStr(edEX_FECHAACCIDENTE.Date)]));
    Verificar(edEX_FECHAEXPUESTO.Date > edEX_FECHAMANIFESTACION.Date, edEX_FECHAEXPUESTO, Format('La fecha de exposición no puede ser posterior a la fecha de primera manifestación %s.', [DateToStr(edEX_FECHAMANIFESTACION.Date)]));
  end;
  ...
  Verificar(not Is_RelacionLaboralActual_o_Historica(EmpresaSeleccionada.CUIT, fraEX_IDTRABAJADOR.CUIL), fraEX_IDTRABAJADOR.mskCUIL,
            'El trabajador no pertenece ni perteneció nunca a la empresa ' + EmpresaSeleccionada.NombreEmpresa + ' en nuestra base de datos.');
  Verificar((cmbEX_FECHARECEPCION.Date <> 0) and (cmbEX_FECHARECEPCION.Date < FechaAccidenteRecaida), edEX_FECHAACCIDENTE, Format('La fecha del accidente/recaída %s no puede ser mayor que la fecha de recepción.', [DateToStr(FechaAccidenteRecaida)]));
  ...
  Verificar(chkEX_EMPLEADOPCP.Checked and not(ExisteRelaLugarTrabajo(fraEX_IDTRABAJADOR.IdTrabajador)), chkEX_EMPLEADOPCP, 'No se puede chequear Empleado PCP debido a que no existe la relación laboral con el establecimiento.' );  // TK 46558
  ...
  Verificar(bMarcando and AreIn(sdqDatosEX_ESTADO.AsString, ['99', '22', '07']), rpNL_MARCADO,
            'No se puede marcar como enfermedad no listada a siniestros con éste estado.');
  ...
  Verificar(bMarcando and (sdqDatosEX_TIPO.AsString <> '3'), rpNL_MARCADO,
            'No se puede marcar como enfermedad no listada a siniestros con éste tipo de contingencia.');
  ...
  Verificar((rpNL_MARCADO.ItemIndex = 1) and not(ExisteSqlEx(sSql, [IdExpediente])),
             rpNL_MARCADO, 'No se puede desmarcar como enfermedad no listada si todavía no fué marcada como tal.');
...
begin
  ...
  try
    ...
    begin
      ...
      try
        ...
        begin
          ...
          edEX_OBSERVACIONES.Lines.Text := 'CECAP N°' + FieldByName('MO_ID').AsString + #13#10 + 'Fecha y hora de recepción de la denuncia: ' + FormatDateTime('dd/mm/yyyy HH:nn', FieldByName('MO_PEDIDO_FECHAHORA').AsdateTime) + ' hs.';
    ...
    InfoHint(fraEX_TIPO.edCodigo, 'Ocurrió un error al recuperar ciertos datos del CEM, por favor avise a Sistemas.');
  end;
...
begin
  ...
  // Para guardar las observaciones (tengo que ponerlo en un solo registro con el mismo idexpediente) así que elijo el ultimo:
