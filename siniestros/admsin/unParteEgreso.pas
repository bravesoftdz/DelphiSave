...
begin
  ...
  if sdqDatos.RecordCount > 1 then
    InfoHint(edPE_DIAGNOSTICO, 'La consulta devolvió mas de un parte de egreso. Si este dato no es correcto consulte a sistemas.');
...
begin
  ...
  begin
    ...
    InfoHint(edPE_FECHAALTALABORAL, 'Alta de recalificación');
  end;
end;
...
const
  ...
  Msg_19 = 'La fecha de baja médica es anterior a la fecha de alta médica de una recaída anterior %s.';
  Msg_20 = 'La fecha reconsulta no puede ser anterior a la fecha de baja médica %s.';
  ...
  Msg_36 = 'La fecha de inicio de internación no puede ser anterior a la fecha de baja médica %s.';
...
begin
  ...
  Verificar(EsPacienteInternado, nil, 'No se puede ingresar el Parte de Egreso: El paciente aún se encuentra internado.');  // TK 24587
...
//  IniciarEspera('Validando...');
  try
    ...
    Verificar((edPE_FECHABAJAMEDICA.Date <> 0) and (edPE_FECHABAJAMEDICA.Date > dHoy), edPE_FECHABAJAMEDICA, Format(Msg_11, ['baja médica', DateToStr(dHoy)]));
...
//    Verificar((edPE_FECHABAJAMEDICA.Date <> 0) and (edPE_FECHABAJAMEDICA.Date > FechaAltaDocumento), edPE_FECHABAJAMEDICA, Format(Msg_13, [DateToStr(FechaAltaDocumento)]));
    ...
    Verificar((fraPE_IDDIAGNOSTICO.Codigo <> 'Z20.9') and
              ...
              (ValorSqlEx(sSQLMix, [IdExpediente]) <> 'S') and  // excluyo los siniestros mixtos de la validación.  TK 18874
    ...
    Verificar((edPE_FECHARECONSULTA.Date <> 0) and (edPE_FECHARECONSULTA.Date < edPE_FECHABAJAMEDICA.Date), edPE_FECHARECONSULTA, Format(Msg_20, ['baja médica', DateToStr(edPE_FECHABAJAMEDICA.Date)]));
    ...
    if rpPE_RECALIFICACION.ItemIndex = 1 then // No sugiere recalificación...
    ...
    Verificar ((edPE_ALTAODONTDEF.Date > 0)  and (edPE_ALTAODONTDEF.Date < edPE_FECHAALTAMEDICA.Date),  edPE_ALTAODONTDEF,  'La Fecha de Alta Odontológica Definitiva, no puede ser menor a la Fecha de Alta Médica.');
    Verificar ((edPE_ALTADERMATDEF.Date > 0) and (edPE_ALTADERMATDEF.Date < edPE_FECHAALTAMEDICA.Date), edPE_ALTADERMATDEF, 'La Fecha de Alta Dermatológica Definitiva, no puede ser menor a la Fecha de Alta Médica.');
    Verificar ((edPE_ALTAPSICOLDEF.Date > 0) and (edPE_ALTAPSICOLDEF.Date < edPE_FECHAALTAMEDICA.Date), edPE_ALTAPSICOLDEF, 'La Fecha de Alta Psicológica Definitiva, no puede ser menor a la Fecha de Alta Médica.');
    ...
    Verificar((edPE_FECHABAJAMEDICA.Date <> 0) and (edPE_FECHARECEPCION.Date < edPE_FECHABAJAMEDICA.Date), edPE_FECHARECEPCION, Format(Msg_14, ['baja médica', DateToStr(edPE_FECHABAJAMEDICA.Date)]));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FieldByName('PE_IDTIPODEEGRESO').AsString := '4'; //Defunción
        //if MostrarAyudasVisuales then
          InfoHint(fraPE_IDGRAVEDAD.edCodigo, 'Cuando el trabajador fallece no hay fecha de alta laboral, ni fecha de reinicio laboral, se omiten algunas preguntas y el tipo de alta es alta por defunción.');
      end else
        if Pos(fraPE_IDTIPODEEGRESO.Codigo{FieldByName('PE_IDTIPODEEGRESO').AsString}, '456') = 0 then
          InfoHint(fraPE_IDTIPODEEGRESO.edCodigo, 'El tipo de egreso no puede ser defunción cuando la gravedad no es mortal.');
    end else
    begin
...
begin
  ...
  Verificar(edPV_FECHACONTROL.Date < FechaAccidenteRecaida, edPV_FECHACONTROL, Format('La fecha de control no puede ser anterior a la del accidente/recaída %s.', [DateToStr(FechaAccidenteRecaida)]));
