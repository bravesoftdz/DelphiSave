inherited fraSAU_AUTORIZACIONES: TfraSAU_AUTORIZACIONES
  Width = 114
  Height = 46
  object btnBusqueda: TSpeedButton [0]
    Left = 93
    Top = 0
    Width = 20
    Height = 21
    Anchors = [akTop, akRight]
    Flat = True
    Glyph.Data = {
      
      0AFAFAFAFAFAFAFAFAFAFAFAFAF8F8F8F8FAFAFAFAFAFAFAFAFA}
    NumGlyphs = 2
  end
  object cmbAU_NUMAUTO: TDBIncComboGrid [1]
    Left = 0
    Top = 0
    Width = 91
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 0
    OnKeyPress = cmbAU_NUMAUTOKeyPress
    Pattern = '0123456789'
    KeyField = 'AU_NUMAUTO'
    ListField = 'AU_NUMAUTO'
    ListAlive = True
    ListSource = dsDatos
    Columns = <
      item
        Expanded = False
        FieldName = 'AU_NUMAUTO'
        Title.Caption = 'N'#250'mero'
        Width = 42
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CA_DESCRIPCION'
        Title.Caption = 'Prestador'
        Width = 120
        Visible = True
      end>
  end
  inherited sdqDatos: TSDQuery
    SQL.Strings = (
      
        'SELECT AU_NUMAUTO, AU_IDENTIFAPRO, AU_FECHASOLI, CA_DESCRIPCION,' +
        ' PR_DESCRIPCION, AU_DETALLEAPRO, GP_NOMBRE, AU_OBSERVACIONES, AU' +
        '_USUAPRO, AU_FECHAPRO FROM CPR_PRESTADOR, MPR_PRESTACIONES, MGP_' +
        'GTRABAJO, SAU_AUTORIZACIONES WHERE CA_IDENTIFICADOR =  AU_IDENTI' +
        'FAPRO AND PR_CODIGO =  AU_PRESAPRO AND GP_CODIGO =  AU_TRABAJO A' +
        'ND AU_ESTADO = '#39'E'#39
      ' AND 1=2')
  end
end
