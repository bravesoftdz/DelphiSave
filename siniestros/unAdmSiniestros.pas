    ...
    edFechaAccidente.Date := fraTrabajador.sdqDatos.FieldByName('EX_FECHAACCIDENTE').AsDateTime;
{
    ...
    try
      ...
      if Trim(edMEDIACIONES.Text) > '' then
        InfoHint(edMEDIACIONES, 'Existe una o más mediaciones para el siniestro seleccionado.');
      ...
      if (fEgreso <> '') and (StrToDate(fEgreso) >= FieldByName('ex_fechaaccidente').AsDateTime) then
        InfoHint(fraTrabajador, 'El trabajador egresó de la empresa');
    ...
    begin
      if HasRecaidasAnterioresRechazadas(Siniestro, Orden, Recaida) {and ActiveForm.MostrarAyudasVisuales} then
        InfoHint(fraTrabajador.edSiniestro, 'Existen para el siniestro recaídas anteriores rechazadas.');
      ...
      if not IsTrabajadorDeclarado(Siniestro, Orden, Recaida) and fraEmpresa.IsSelected {and ActiveForm.MostrarAyudasVisuales} then
        InfoHint(fraEmpresa.mskCUIT, 'El empleador omitió declarar su obligación de pago. Pagos a recuperar del empleador.'{, False, 'Información', blnInfo, 10});
    end;
  ...
  begin
    ...
    edMEDIACIONES.Clear;
    edMEDIACIONES.Hint := '';
    pnlStatus.Caption := 'Ud. no está visualizando un siniestro';
...
begin
  ...
  begin
    ...
    begin
      ...
      sExtra := CRLF + 'Tenga en cuenta que la imagen está asociada a un siniestro diferente!' + CRLF + '(' + Armar_Siniestro(nSin, nOrd, nRec) + ')';
    end;
    ...
    if (not sdqImagenes.FieldByName('TD_CODIGO').IsNull) and (sdqImagenes.FieldByName('TD_CODIGO').AsString <> GetTipoDocumento(ACodigoDocumento)) then
      sExtra := sExtra + CRLF + 'Tenga en cuenta que la imagen está asociada a un documento diferente!' + CRLF + '(' + sdqImagenes.FieldByName('TD_DESCRIPCION').AsString + ')';
      ...
      begin
        try
          ...
          try
            ...
            if bCopiaOk then
              begin
                ...
                if IdDocumento = ART_EMPTY_ID then
                  raise Exception.Create('El código de documento ' + GetTipoDocumento(ACodigoDocumento) + ' no se encuentra definido en el sistema Adm. Gral. de Archivos, para el tipo de archivo: Siniestros.')
                else
                  begin
            ...
            begin
              RollBack;
              MsgBox('Ocurrió un error al digitalizar la imagen actual, consulte con sistemas sobre el siguiente error.' + CRLF + E.Message);
            end;
          end;
        finally
          VerStats;
...
begin
  ...
  begin
    ...
    begin
      if not MsgAsk('Este procedimiento recargará los datos originales perdiendo los últimos cambios.' + #13#10 + '¿Desea continuar?') then
        Abort;
    end;
    ActiveForm.Refresh;
    DesbloqueoFiltros;
...
begin
//  Verificar(fraTrabajador.IsEmpty, fraTrabajador.mskCUIL, 'No ha seleccionado ningún trabajador para ver sus datos.');
...
begin
  if TieneImagenesBloquedasRecientemente(ActiveForm.IDExpediente, Sesion.UserID, GetTipoDocumento) then
    sAuxiliar := '?' + CRLF + CRLF + 'Tenga en cuenta que no se generará registro alguno de digitalización pues' + CRLF + 'Ud. tiene bloqueada una imagen de este siniestro actualmente.'
  else
    sAuxiliar := CRLF + 'y generar su correspondiente registro de digitalización?';
...
begin
  ...
  begin
    ...
    begin
      ...
      MsgBox('El sistema ha detectado que no quedan más imágenes en el repositorio,' + CRLF + 'teniendo en cuenta las bandejas a las que Ud. está suscripto.');
    end;
        ...
        try
          ...
          begin
            ...
            case MessageBox(Application.Handle, PChar(Fields[1].AsString + CRLF +
                            ...
                            'Página ' + sdqImagenes.FieldByName('FE_NROPAGINA').AsString + '/' + FieldByName('CANT_PAG').AsString + CRLF +
                            sdqImagenes.FieldByName('EXTRA').AsString + 
                            '¿Desea comenzar la carga de este documento?'),
                            'Carga de imágenes digitalizadas', MB_ICONQUESTION or MB_YESNOCANCEL) of
              idYes:
                begin
                  pnlFechaDoc.Visible := True;
                  Break;
              ...
              idNo:
                begin
                  if MsgAsk('Está a punto de enviar el papel actual a la bandeja de pendientes.' + CRLF + '¿Desea continuar?') then
                  begin
                    EjecutarSQLEx('UPDATE SIN.SGI_GESTIONIMAGENES ' +
...
begin
  ...
  try
    IDExpediente := ActiveForm.IdExpediente;
    lbSubTitulo.Caption := 'Esta es la ubicación de la denuncia del siniestro ' + fraTrabajador.edSiniestro.SinOrdRec;
