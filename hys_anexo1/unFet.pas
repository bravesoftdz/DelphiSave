...
begin
  ...
  begin
    Verificar(edFechaRecepcion.Date < ValorSqlDateTime(
          ...
          '    AND VO_OPERATIVO = '+SqlValue(fraOperativo.Codigo)),edFechaRecepcion,'La fecha de recepción debe ser mayor que la de notificación de ET/EP');
  end;
  ...
  *)
  ...
  begin
    ...
    begin
      ...
      InvalidMsg(FFrmPRS, 'El diagnóstico ' + Valor + ' no tiene una recomendación asociada.');
    ...
    begin
      PageControl.ActivePageIndex := TAB_PRS;
      InvalidMsg(FFrmPRS, 'La recomendación ' + Valor + ' no tiene un diagnóstico asociado.');
      exit;
    end;
  end;
...
//                              ' AND OE_FECHABAJA IS NULL ' +
  ...
  begin
    ...
    begin
      ...
      InvalidMsg(tsPRS, 'El mínimo de visitas por plan es de cuatro (4).');
      Exit;
    end;
    }
    ...
    begin
      PageControl.ActivePageIndex := 0;
      InvalidMsg(gbAmeriraPRS, 'Debe marcar una opción.');
      Exit;
    end;
  end;
...
//  comparar los 3 boolean
  ...
  begin
    ...
    begin
      ...
      begin
        tsAnexos.ActivePage := tsAnexo1;
        InvalidMsg(edFechaRecepcion, 'Se debe completar la fecha de recepción en Anexo 1.');
        Exit;
      end;
      if (fraTipoFet.Value = 'ET') and (trim(edTelefono.Text) = '') then
      begin
        tsAnexos.ActivePage := tsAnexo1;
        InvalidMsg(edTelefono, 'Se debe completar el teléfono del establecimiento.');
        Exit;
      end;
      ...
      begin
        tsAnexos.ActivePage := tsAnexo1;
        InvalidMsg(edEMail, 'El Email del establecimiento es inválido.');
        Exit;
      end;
      ...
      begin
        tsAnexos.ActivePage := tsAnexo1;
        InvalidMsg(edFechaRecepcion, 'Se debe completar la fecha de recepción en Anexo 3.');
        Exit;
      end;
    ...
    begin
      tsAnexos.ActivePage := tsAnexo1;
      InvalidMsg(edFechaNotificacionAnexo, 'Se debe completar la fecha de Notificación.');
      Exit;
    end;
    ...
    begin
      tsAnexos.ActivePage := tsAnexo1;
      InvalidMsg(ScrollBox, 'El Anexo 1 está incompleto.');
      Exit;
    end;
...
begin
     if fraEstableciBusq.edCodigo.Value <> 0 Then begin
         Sql := 'SELECT ''Teléfono de la Empresa: '' || MP_TELEFONOS || chr(13) || ''Teléfono del Establecimiento: '' || ST_TELEFONOS ' +
         ...
         MsgBox(ValorSql(Sql), MB_ICONINFORMATION, 'Información del Establecimiento');
     end;
end;
{----------------------------------------------------------------------------------------------------------------------}
...
//    tsAnexo2.Caption := 'Anexo 2';
    ...
    Case ArrayPos( fraTipoFet.Value, ['ET', 'EG', 'EA', 'EP'] ) of
      EMPRESA_TESTIGO: begin
        ...
        begin
          lbItem5.Caption := '5 - ¿Cúantas enfermedades profesionales han denunciado a la ART durante el año 2000?';
          lbItem6.Caption := '6 - ¿Cuántos trabajadores siniestrados pertencientes al establecimiento denunció a la ART durante el año 2000?';
          lbItem6_1.Caption := '6.1 - ¿Cuántos de ellos se encontraban trabajando en el momento del accidente?';
          lbItem6_1_1.Caption := '6.1.1 - ¿Cuántos de estos trabajadores se ausentaron de su puesto de trabajo al menos un día y cuántos fallecieron?';
          ...
          lbItem5.Caption := '5 - ¿Ha denunciado la existencia de alguna profesional a la ART durante el último año?';
          lbItem6.Caption := '6 - ¿Cuántos trabajadores siniestrados se denunciaron a la ART durante el último año?';
          lbItem6_1.Caption := '6.1 - De ellos, ¿cuántos se encontraban trabajando en el momento del accidente?';
          lbItem6_1_1.Caption := '6.1.1 - ¿Cuántos de estos trabajadores se ausentaron de su puesto de trabajo al menos 11 días o fallecieron?';
          lbItem6_1_1_A.Caption := 'Siniestros > 10 días';
        ...
        lbItem4.Caption := '4 - ¿Cuántas personas trabajaron en promedio los últimos seis meses en el establecimiento?';
        tsAnexo1.Caption := 'Anexo 1';
        nbItem4.ActivePage := 'Anexo 1';
        tsAnexo2.Caption := 'Anexo 2';
        tsPRS.Caption    := 'PRS';
      ...
      EMPRESA_GUIA: begin
        lbItem4.Caption := '4 - ¿Cuál es el promedio de trabajadores en el establecimiento durante el 2001?';
        lbItem5.Caption := '5 - ¿Cúantas enfermedades profesionales han denunciado a la ART durante el año 2001?';
        lbItem6.Caption := '6 - Cuántos trabajadores siniestrados pertencientes al establecimiento denunció a la ART durante el año 2001?';
        lbItem6_1.Caption := '6.1 - ¿Cuántos de ellos se encontraban trabajando en el momento del accidente?';
        lbItem6_1_1.Caption := '6.1.1 - ¿Cuántos de estos trabajadores se ausentaron de su puesto de trabajo al menos un día y cuántos fallecieron?';
        lbItem6_1_1_A.Caption := 'A - Siniestros con Ausencia :';
        lbItem6_1_1_B.Caption := 'B - Fallecidos :';
      ...
      EMPRESA_AGRICOLA : begin
        lbItem4.Caption := '4 - ¿Cuál es la mayor cantidad de trabajadores declarados en un mes?';
        lbItem5.Caption := '5 - ¿Cúantas enfermedades profesionales han denunciado a la ART durante el año 2001?';
        lbItem6.Caption := '6 - Cuántos trabajadores siniestrados pertencientes al establecimiento denunció a la ART durante el año 2001?';
        lbItem6_1.Caption := '6.1 - ¿Cuántos de ellos se encontraban trabajando en el momento del accidente?';
        lbItem6_1_1.Caption := '6.1.1 - ¿Cuántos de estos trabajadores se ausentaron de su puesto de trabajo al menos un día y cuántos fallecieron?';
      ...
      EMPRESA_PYMES: begin
        lbItem4.Caption := '4 - ¿Cuenta con el relevamiento de agentes de riesgos para la detección precoz de Enfermedades Profesionales?';
        lbItem5.Caption := '5 - ¿Ha denunciado la existencia de alguna enfermedad profesional a la ART durante el año calendario anterior?';
        lbItem6.Caption := '6 - Cuántos trabajadores siniestrados pertencientes al establecimiento denunció a la ART el año calendario anterior?';
        lbItem6_1.Caption := '6.1 - ¿Cuántos de ellos se encontraban trabajando en el momento del accidente?';
        lbItem6_1_1.Caption := '6.1.1 - ¿Cuántos de estos trabajadores se ausentaron de su puesto de trabajo al menos un día y cuántos fallecieron?';
        lbItem6_1_1_A.Caption := 'A - Siniestros con Ausencia :';
        lbItem6_1_1_B.Caption := 'B - Fallecidos :';
...
begin
  ...
  begin
      ...
      if (trim(edCPA.Text) = '') then
        InvalidMsg(edCPA, 'El establecimiento no tiene CPA y no será exportado a la SRT.', 'Validación CPA', MB_ICONWARNING);
      LoadData;
...
begin
 ...
 { else  Qry := GetQuery('SELECT RE_SEGUIMIENTO, RE_RECOMENDACION ' +
  ...
  try
    ...
    begin
        RecomVencidas := RecomVencidas + 'Recomendación: ' + Qry.FieldByName('RE_RECOMENDACION').AsString + ' - Vencimiento: '
  ...
  result := (trim(RecomVencidas) = '') or
            (MsgBox('Las siguientes recomendaciones están vencidas: '+ #13#10  + RecomVencidas + #13#10 + '¿Desea guardar los datos de todas formas? ',
...
begin
  ...
  begin
    ...
    begin
      ...
      Excavacion := 'Excavación: '+frmObras.edFechaExcavacion.Text+ ' hasta '+ frmObras.edFechaExcavacionHasta.Text+sExtra+#13#10;
    end;
    ...
    begin
      ...
      demolicion := 'Demolición: '+frmObras.edFechaDemolicion.Text + ' hasta '+frmObras.edFechaDemolicionHasta.Text+sExtra+#13#10;
    end;
    ...
    begin
      ...
      'poseen tareas de "EXCAVACION" y/o "DEMOLICION" que deberán ser visitadas dentro '+
      'de los siete (7) días corridos y/o a las cuarenta y ocho (48) horas, respectivamente, '+
      'de iniciados los trabajas. '+#13#10+
      'A partir del día de la fecha ya pueden ser contactadas, las razones sociales, para '+
      'coordinar la visita al establecimiento en cuestión. '+#13#10+#13#10+
      ...
      'de RIESGO ALTO, y que por lo tanto deberán ser visitadas ateniéndose al cronograma designado.' + #13#10 +
      'A partir del día de la fecha ya pueden ser contactadas, las razones sociales, para coordinar ' +
      'la visita al establecimiento en cuestión. '+#13#10+#13#10+
      'Datos de las tareas: '+#13#10+
      fraEmpresa.CUIT+'-'+ValorSql('SELECT em_nombre FROM afi.aem_empresa WHERE em_cuit ='+SqlValue(fraEmpresa.CUIT))+#13#10+
      'Establecimiento: '+fraEstableciBusq.edCodigo.Text+#13#10+
      'Dirección: '+Direccion+#13#10+
    ...
    if (Excavacion > '') and  (demolicion > '') then
     sAsunto := 'Visita Obra - Excavación/Demolición - con RIESGO ALTO'
    else
    begin
      if (Excavacion > '') then
        sAsunto := 'Visita Obra - Excavación - con RIESGO ALTO'
      else
      begin
        if (demolicion > '') then
          sAsunto := 'Visita Obra - Demolición - con RIESGO ALTO'
        else
...
begin
     if not IsCuit(mskCUITContrante.text) Then
        InvalidMsg(mskCUITContrante, 'Debe ingresar un CUIT válido.')
...
(*
  if (Sesion.LoginName = 'GIÑIGUEZ') or (Sesion.LoginName = 'LBASTITTA') then
...
//  Verificar(ValorSql('SELECT art.amebpba.isdiahabil('+SqlValue(edFechaVisita.Date)+') FROM DUAL')= 'N',edFechaVisita,'La Fecha no es un día habil.');
  ...
  (*Verificar((motivoPos >=0) and   cbMotivos.Checked[motivoPos] and  ExisteSql('SELECT 1 '+
                            ...
                            '   AND rv_id <> '+SqlValue(sdqVisitas.FieldByName('rv_id').AsString))),cbMotivos,'Ya existe una visita con el motivo Notificación ET.');
  *)
  ...
  begin
    Verificar(edFechaViaticos.Date = 0,edFechaViaticos,'Debe ingresar Fecha Entrega de Viáticos.');
  ...
  begin
    Verificar(edFechaFinViaticos.Date < edFechaVisita.Date, edFechaFinViaticos, 'La fecha hasta de viáticos debe ser mayor o igual a la fecha de visita.' )
  end;
  ...
  begin
    Verificar(edFechaInicioViaticos.Date > edFechaVisita.Date, edFechaInicioViaticos, 'La fecha desde de viáticos debe ser menor o igual a la fecha de visita.' )
  end;
  }
...
begin
  ...
  begin
    for i := 0 to cbMotivos.Items.Count - 1 do
      // Si está seleccionado, agrego si no existe
    ...
    begin
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              //campo 4 Causal/riesgo
                ...
                begin
                  MsgBox('No hay consecutividad, se guardo hasta el Diagnóstico '+IntToStr(diagAnt));
        ...
        {*****************************************************************************}
                  ...
                  begin
                    if not error then
                      listError.Add('#Diagnóstico.');
                    error := true;
                    listError.Add('#Debe especificar el tipo de Diagnóstico.');
                  end;
                  ...
                  begin
                    if not error then
                      listError.Add('#Diagnóstico.');
                    error := true;
                    listError.Add('#Debe especificar la Descripción del Diagnóstico.');
                  end;
        {******************************************************************************}
                  if not error then
                  begin
                ...
                //campo 5 CA y RPAsociados
                  ...
                  begin
                    MsgBox('No hay consecutividad, se guardo hasta la Recomendación '+IntToStr(recAnt));
                    errorConsecutividad := True;
                  end
                  else
                  begin
                  ...
                  {*****************************************************************************}
                    ...
                    begin
                      if not error then
                        listError.Add('#Recomendación.');
                      error := true;
                      listError.Add('#Debe especificar el responsable de ejecución.');
                    end;
                    if (trim(campos.Strings[1]) = '') then
                    begin
                      if not error then
                        listError.Add('#Recomendación.');
                      error := true;
                      listError.Add('#Debe especificar la Descripción de la Recomendación.');
                    end;
                    ...
                    begin
                      if not error then
                        listError.Add('#Recomendación.');
                      error := true;
                      listError.Add('#El rango de fechas es inválido.');
                    end;
                    if (trim(campos.Strings[1]) = '') then
                    begin
                      if not error then
                    ...
                    begin
                      if not error then
                        listError.Add('#Recomendación.');
                      error := true;
                      listError.Add('#El límite de diagnósticos por recomendación es 10.');
                    end;
                    ...
                    begin
                      if not error then
                        listError.Add('#Recomendación.');
                      error := true;
                      listError.Add('#Debe especificar al menos un diagnóstico');
                    end;
                    ...
                    begin
                      ...
                      begin
                        if not error then
                          listError.Add('#Recomendación.');
                        error := true;
                        listError.Add('#El diagnóstico ' +campos.Strings[j]+' no existe.');
                      end;
                    end;
                  {*****************************************************************************}
                    if not error then
