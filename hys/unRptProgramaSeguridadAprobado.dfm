object rptProgramaSeguridadAprobado: TrptProgramaSeguridadAprobado
  Left = 336
  Top = 187
  Width = 979
  Height = 563
  VertScrollBar.Position = 520
  Caption = 'Aviso de Programa de Seguridad Aprobado'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Scaled = False
  PixelsPerInch = 96
  TextHeight = 13
  object qrResolucion: TQuickRep
    Left = 1
    Top = -520
    Width = 816
    Height = 1056
    Frame.Color = clBlack
    Frame.DrawTop = False
    Frame.DrawBottom = False
    Frame.DrawLeft = False
    Frame.DrawRight = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    Functions.Strings = (
      'PAGENUMBER'
      'COLUMNNUMBER'
      'REPORTTITLE')
    Functions.DATA = (
      '0'
      '0'
      #39#39)
    IDReport = 0
    Options = [FirstPageHeader, LastPageFooter]
    Page.Columns = 1
    Page.Orientation = poPortrait
    Page.PaperSize = Letter
    Page.Values = (
      100.012500000000000000
      2794.000000000000000000
      100.012500000000000000
      2159.000000000000000000
      100.012500000000000000
      100.012500000000000000
      0.000000000000000000)
    PrinterSettings.Copies = 1
    PrinterSettings.OutputBin = Auto
    PrinterSettings.Duplex = False
    PrinterSettings.FirstPage = 0
    PrinterSettings.LastPage = 0
    PrinterSettings.ExtendedDuplex = 0
    PrinterSettings.UseStandardprinter = False
    PrinterSettings.UseCustomBinCode = False
    PrinterSettings.CustomBinCode = 0
    PrinterSettings.UseCustomPaperCode = False
    PrinterSettings.CustomPaperCode = 0
    PrinterSettings.PrintMetaFile = False
    PrintIfEmpty = True
    ReportTitle = 'Aviso de Programa de Seguridad Aprobado'
    SnapToGrid = True
    Units = Native
    Zoom = 100
    PrevFormStyle = fsNormal
    PreviewInitialState = wsNormal
    object QRBand1: TQRBand
      Left = 38
      Top = 38
      Width = 740
      Height = 236
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        624.416666666666800000
        1957.916666666667000000)
      BandType = rbTitle
      object logo: TQRImage
        Left = 586
        Top = 1
        Width = 152
        Height = 60
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          158.750000000000000000
          1550.458333333333000000
          2.645833333333333000
          402.166666666666600000)
        AutoSize = True
        Picture.Data = {
          
          FFFF}
        Stretch = True
      end
      object QRLabel1: TQRLabel
        Left = 4
        Top = 62
        Width = 35
        Height = 16
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          42.333333333333340000
          10.583333333333330000
          164.041666666666700000
          92.604166666666680000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Sres.'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel2: TQRLabel
        Left = 4
        Top = 80
        Width = 58
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          211.666666666666700000
          153.458333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Contrato'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel3: TQRLabel
        Left = 388
        Top = 80
        Width = 34
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          1026.583333333333000000
          211.666666666666700000
          89.958333333333320000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CUIT'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel4: TQRLabel
        Left = 4
        Top = 103
        Width = 33
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          272.520833333333300000
          87.312500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CIUU'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel5: TQRLabel
        Left = 4
        Top = 136
        Width = 179
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          359.833333333333300000
          473.604166666666700000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Ref: Programa de Seguridad'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrEncabezado: TQRMemo
        Left = 4
        Top = 154
        Width = 732
        Height = 76
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          201.083333333333300000
          10.583333333333330000
          407.458333333333400000
          1936.750000000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = True
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          'De nuestra consideraci'#243'n:'
          
            '       Con referencia a vuestra consulta sobre la condici'#243'n en l' +
            'a que se encuentra la documentaci'#243'n para la direcci'#243'n de DOMICIL' +
            'IO - LOCALIDAD, con fecha de inicio en FECHA DECLARADA y FECHA F' +
            'INALIZACI'#211'N DE LA ACTIVIDAD DE LA OBRA, por ud. entregada, le in' +
            'formamos que la misma se encuentra (APROBADO)')
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlRazonSocial: TQRLabel
        Left = 89
        Top = 62
        Width = 99
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          235.479166666666700000
          164.041666666666700000
          261.937500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlRazonSocial'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlContrato: TQRLabel
        Left = 89
        Top = 80
        Width = 76
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          235.479166666666700000
          211.666666666666700000
          201.083333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlContrato'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlCUIT: TQRLabel
        Left = 427
        Top = 80
        Width = 52
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          1129.770833333333000000
          211.666666666666700000
          137.583333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlCUIT'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlCIUU: TQRLabel
        Left = 90
        Top = 103
        Width = 49
        Height = 16
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          42.333333333333340000
          238.125000000000000000
          272.520833333333300000
          129.645833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlCIUU'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
    object QRBand2: TQRBand
      Left = 38
      Top = 665
      Width = 740
      Height = 148
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        391.583333333333400000
        1957.916666666667000000)
      BandType = rbSummary
      object qriFirma: TQRImage
        Left = 413
        Top = 6
        Width = 304
        Height = 101
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          267.229166666666700000
          1092.729166666667000000
          15.875000000000000000
          804.333333333333200000)
        AutoSize = True
        Center = True
      end
      object qrlFirmante: TArtQRLabel
        Left = 417
        Top = 110
        Width = 100
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1103.312500000000000000
          291.041666666666700000
          264.583333333333400000)
        Alignment = taCenter
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Nombre Firmante'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRLabel6: TQRLabel
        Left = 418
        Top = 129
        Width = 255
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1105.958333333333000000
          341.312500000000000000
          674.687500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = #193'rea Construcci'#243'n - Gerencia de Prevenci'#243'n'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 8
      end
    end
    object QRBand3: TQRBand
      Left = 38
      Top = 274
      Width = 740
      Height = 391
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        1034.520833333333000000
        1957.916666666667000000)
      BandType = rbDetail
      object QRMemo1: TQRMemo
        Left = 4
        Top = 5
        Width = 732
        Height = 380
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          1005.416666666667000000
          10.583333333333330000
          13.229166666666670000
          1936.750000000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = True
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          
            'Este mismo documento le sirve a ud., como acuse de recibo ante l' +
            'a presentaci'#243'n y aprobaci'#243'n de los documentos a los cuales aqu'#237' ' +
            'se hace referencia.'
          ''
          'Tenga en cuenta que:'
          ''
          
            '       La Resoluci'#243'n SRT N'#186' 51/97 establece en su art'#237'culo 2'#186': '#8220 +
            'Establ'#233'cese que, a partir de la fecha de publicaci'#243'n de la prese' +
            'nte, los empleadores de la construcci'#243'n, adem'#225's de la notificaci' +
            #243'n dispuesta por el art'#237'culo 1'#186' de la presente resoluci'#243'n, deber' +
            #225'n confeccionar el Programa de seguridad que integra el Legajo T' +
            #233'cnico, seg'#250'n lo dispuesto por la Resoluci'#243'n SRT N'#186' 231/96, Anex' +
            'o I, art'#237'culo 3'#186', para cada obra que inicien, que se adjuntar'#225' a' +
            'l contrato de afiliaci'#243'n, cuando las mismas tengan alguna de las' +
            ' siguientes caracter'#237'sticas: a) excavaci'#243'n; b) demolici'#243'n; c) co' +
            'nstrucciones que indistintamente superen los UN MIL METROS CUADR' +
            'ADOS (1000 m) de superficie cubierta o los CUATRO METROS (4 m) d' +
            'e altura a partir de la cota CERO (0); d) tareas sobre o en prox' +
            'imidades de l'#237'neas o equipos energizados con Media o Alta Tensi'#243 +
            'n, definidas MT y AT seg'#250'n el Reglamento del ENTE NACIONAL REGUL' +
            'ADOR DE LA ELECTRICIDAD (E.N.R.E.); e) en aquellas obras que, de' +
            'bido a sus caracter'#237'sticas, la Aseguradora del empleador lo cons' +
            'idere pertinente.'#8221' '
          ''
          
            '       Del mismo modo, la Resoluci'#243'n 319/99 dice en su art'#237'culo ' +
            '6'#186': Establ'#233'cese que los empleadores que realicen obras de car'#225'ct' +
            'er repetitivo y de corta duraci'#243'n, y cuyos trabajos se encuentre' +
            'n comprendidos en el art'#237'culo 2'#186' de la Resoluci'#243'n '
          
            'SRT N'#186' 51/97, confeccionar'#225'n y presentar'#225'n ante su ART, un Progr' +
            'ama de seguridad de acuerdo a lo indicado en dicha Resoluci'#243'n, c' +
            'on los contenidos, mecanismos y validez que se establecen en el ' +
            'Anexo II de la presente resoluci'#243'n.'
          ''
          
            '      El decreto 911/96 reglamentario de la Higiene y Seguridad ' +
            'de la actividad de la construcci'#243'n determina que: '#8220'A los efectos' +
            ' de este Decreto, se incluye en el concepto de obra de construcc' +
            'i'#243'n a todo trabajo de ingenier'#237'a y arquitectura realizado sobre ' +
            'inmuebles, propios o de terceros, p'#250'blicos o privados, comprendi' +
            'endo excavaciones, demoliciones, construcciones, remodelaciones,' +
            ' mejoras, refuncionalizaciones, grandes mantenimientos, montajes' +
            ' e instalaciones de equipos y toda otra tarea que se derive de, ' +
            'o se vincule a, la actividad principal de las empresas construct' +
            'oras.'#8221)
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
  end
end
