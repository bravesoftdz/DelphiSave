...
type
  TfrmInvRptBajasCartera = class(TfrmCustomForm)
...
var
  frmInvRptBajasCartera: TfrmInvRptBajasCartera;
...
procedure TfrmInvRptBajasCartera.btnAceptarClick(Sender: TObject);
var
  sSql: String;
begin
  ...
  sSql := 'SELECT OI_FECHA FECHA, TO_DESCRIPCION OPERACION, OI_FECHACUSTODIA LIQUIDA, IN_CODCAJAVALORES COD_CAJA_VALORES, ' +
                 ...
                 'ART.INVERSION.GET_COSTOOPERACION(OI_ID) COSTO, NVL(OI_MONTO, 0) - ART.INVERSION.GET_COSTOOPERACION(OI_ID) RESULTADO ' +
  ...
  with qr do
    begin
      ...
      ReportTitle             := 'Bajas de Cartera';
      qrlParam1.Caption       := fraTipoInstrumento.Descripcion;
      qrlParam2.Caption       := 'LISTADO DE BAJAS DESDE: ' + DateToStr(edFechaDesde.Date) + ' HASTA: ' + DateToStr(edFechaHasta.Date);
...
procedure TfrmInvRptBajasCartera.FormatMonto(Sender: TObject; var Value: String);
begin
  Value := Get_AjusteDecimales(Value, '$');
end;

procedure TfrmInvRptBajasCartera.FormatMonto6(Sender: TObject; var Value: String);
begin
  Value := Get_AjusteDecimales(Value, '$', 6);
end;

procedure TfrmInvRptBajasCartera.FormatMontoSinPesos(Sender: TObject; var Value: String);
begin
  Value := Get_AjusteDecimales(Value, '');
end;

procedure TfrmInvRptBajasCartera.FormCreate(Sender: TObject);
begin
  inherited;
