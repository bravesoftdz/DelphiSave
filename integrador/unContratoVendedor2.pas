...
uses
  unDmPrincipal, unPrincipal, CustomDlgs, General, VCLExtra, Periodos,
...
const
  ...
  CONSULTA_GRILLA: String =
    'SELECT en_id, en_codbanco, en_nombre, ve_id, ve_vendedor, ve_nombre, vc_id, vc_idconcepto, vc_porccomision,' +
     ...
     ' FROM asu_sucursal, xco_concepto, xen_entidad, xve_vendedor, xev_entidadvendedor, PAGO.PVC_VENDEDORCONTRATO' +
...
begin
  ...
  sSql :=
    'SELECT 1' +
     ' FROM PAGO.PVC_VENDEDORCONTRATO' +
  ...
  begin
    sSql :=
      'SELECT 1' +
       ' FROM xev_entidadvendedor, PAGO.PVC_VENDEDORCONTRATO' +
...
begin
  Result := GetSecNextVal('PAGO.SEQ_PVC_ID');

  with aSqlTrans do
  begin
    Clear;
    TableName := 'PAGO.PVC_VENDEDORCONTRATO';
...
begin
  ...
  aSqlTrans := TSql.Create('PAGO.PVC_VENDEDORCONTRATO');
  try
    case Funcion of
      fsAgregar:
      begin
...
//Esto no es el cierre de vigencia: es la baja
  ...
  begin
    ...
    begin
      ...
      begin
        with TSql.Create('PAGO.PVC_VENDEDORCONTRATO') do
        try
          SqlType := stUpdate;
