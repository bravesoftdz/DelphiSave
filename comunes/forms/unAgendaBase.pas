...
begin
  ...
  if ExisteSQLEx(sSQL, [AIdTarea, TDateTimeEx.Create(GetMaxFechaTarea(AIdTarea))]) then
    InfoHint(nil, 'La tarea seleccionada ha cambiado de estado mientras Ud. intentaba realizar una acción sobre ella, ' +
                  'actualice la lista de tareas por favor.', True, 'Validación', blnWarning);
end;
...
begin
  ...
  begin
    Verificar(tvTareasAsignar.SelectionCount <> 1, nil, 'Esta acción solo puede realizarse si se selecciona de a un registro.');
...
begin
  for i := 0 to tvTareasAsignar.SelectionCount - 1 do
    if not Assigned(tvTareasAsignar.Selections[i].Data) then
      InfoHint(nil, 'Por favor no seleccione un nodo de agrupamiento para realizar una derivación masiva.', True, 'Validación', blnWarning);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        MarcarDerivacion(Sesion.UserID, nIdTarea,
                         'Derivación a ' + iif(fraUsuarioConSectorIntranet.fraUsuarioSeleccionado.IsEmpty,
...
begin
  ...
  begin
    ...
    InfoHint(btnEliminar, 'Debe seleccionar un evento para poder darlo de baja.', False, 'Validación', blnWarning);
end;
      ...
      begin
        ...
        begin
          ...
          EjecutarSQLEx('UPDATE agenda.aat_agendatarea ' +
                         ...
                         'WHERE at_id = :id', [nIdSeguimiento, Sesion.ID, {Esto lo hago porque puede ser un jefe quien lo esté tomando...} //TApptTarea(tvTareasAsignar.Selected.Data).IdUsuarioSeguimiento,
                                               nIdTarea]);
...
begin
  Verificar(fraUsuarioConSectorIntranet.IsEmpty, btnDerivar, 'Debe seleccionar al menos un sector para derivar esta tarea.' + CRLF + 'Nota: Puede seleccionar un usuario en particular si así lo desea.');
  Verificar(IsEmptyString(edOBSERVACIONES.Lines.Text), edOBSERVACIONES, 'Debe indicar el motivo de la derivación.');
  Verificar(Length(Trim(edOBSERVACIONES.Lines.Text)) < 5, edOBSERVACIONES, 'Le sugerimos indicar el motivo de la derivación, descríbalo al menos sintéticamente.');
  ...
  begin
    ...
    try
      if not MsgAsk('Se derivará la tarea a los siguientes usuarios: ' + CRLF + CRLF + tslUsuarios.Text + CRLF + CRLF + '¿Desea continuar?') then
        Abort;
...
begin
  ...
  try
    ...
    if Assigned(frmABMAgendaBaseTarea) then
      InfoHint(nil, 'Espere por favor, hay una tarea que se esta procesando. Si este mensaje persiste cierre el módulo de agenda para continuar.', True, 'Atención!', blnWarning);
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if ShowModal = mrOK then
            RefreshTreeView
          else // Marcó como leído el solo hecho de abrir la tarea...
      ...
      InfoHint(btnModificarTarea, 'No seleccionó ninguna tarea para modificar.');
  finally
  ...
  begin
    ...
    begin
      if UsuariosAsignados.Count > 1 then
        ...
        lblUsuariosEventosAsignados.Caption := 'No hay seleccionados (asignación automática)';
...
begin
  ...
  begin
    ...
    if UsuariosAsignados.Count > 1 then
      ...
      lblUsuariosTareasAsignados.Caption := 'No hay seleccionados (asignación automática)';
  end;
  RefreshTreeView;
end;
...
begin
  Verificar((not Assigned(objAppt)) and (AModoABM = abmModificacion), btnModificar, 'No seleccionó ningún evento para visualizar.');
  ModoABM := AModoABM;
  try
    Abrir(TfrmABMAgendaBaseEvento, frmABMAgendaBaseEvento, tmvNormal, nil);
    with frmABMAgendaBaseEvento do
  ...
  for i := 0 to tvTareasAsignar.SelectionCount - 1 do
    ...
    begin
      ...
      try
        ...
        MarcarDerivacion(Sesion.UserID, nIdTarea, 'Asignación de la tarea a ' + StringReplace(TMenuItem(Sender).Caption, '&', '', [rfReplaceAll]));
      ...
      InfoHint(nil, 'No se pudo derivar la tarea.', False, 'Validación', blnWarning);
...
begin
  ...
  try
    ...
    if sdqGetApptQuery.RecordCount = 1 then
      ...
      InfoHint(btnStart, 'Ocurrió un error inesperado, informe al buzón sic@provart.com.ar de este suceso.');
...
begin
  ...
  begin
    ...
    Items[3].GroupTitle := 'Miércoles';
    Items[4].GroupTitle := 'Jueves';
    Items[5].GroupTitle := 'Viernes';
    Items[6].GroupTitle := 'Sábado';
  end;
end;
...
begin
  ...
  sAuxCaseDias := 'CASE ' +
                       ...
                       'WHEN TRUNC(tarea.%campo%) = TRUNC(SYSDATE) + 1 THEN ''Mañana'' ' +
  ...
  case rgAgrupaciones.ItemIndex of
    ...
    6 : AddField(sSQL, 'DECODE (tarea.AT_LEIDO, ''S'', ''Leído'', ''N'', ''No Leído'')', 'AGRUPAR');
    ...
    8 : AddField(sSQL, 'NVL((SELECT tb_descripcion ' +
                             ...
                             'WHERE au_idexpediente = tarea.at_idexpediente and at_numauto = au_numauto ' +
                               ...
                               'AND rp_domicilio = ''S'' ' +
                               'AND tb_clave = ''REGSA''), ''Sin región sanitaria'')', 'AGRUPAR');
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            with tvTareasAsignar.Items.AddChild(tnNodoHijo, 'Esta tarea está tomada por ' + FieldByName('usuario').AsString) do
            begin
