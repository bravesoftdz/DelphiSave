unit unManAPC_PERMITECLONACION;
...
type
  TfrmManAPC_PERMITECLONACION = class(TfrmCustomPanelABM)
...
var
  frmManAPC_PERMITECLONACION: TfrmManAPC_PERMITECLONACION;
...
procedure TfrmManAPC_PERMITECLONACION.FSFormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmManAPC_PERMITECLONACION.LoadGrillaEstados(const aClave: String);
var
  sSql: String;
begin
  sSql :=
...
procedure TfrmManAPC_PERMITECLONACION.tbRefreshClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmManAPC_PERMITECLONACION.tbGuardarClick(Sender: TObject);
var
  i: Integer;
begin
  BeginTrans;
...
procedure TfrmManAPC_PERMITECLONACION.tbImprimirClick(Sender: TObject);
begin
  GridConPermiso.Print;
end;

procedure TfrmManAPC_PERMITECLONACION.AgregarPermitido(const aCodigo, aDescripcion: String; var aTargetGrid: TAdvStringGrid);
begin
  aTargetGrid.AddRow;
...
procedure TfrmManAPC_PERMITECLONACION.LoadGrillaEstadosConPermiso(const aClave: String);
var
  sSql: String;
begin
...
procedure TfrmManAPC_PERMITECLONACION.btnAgregarClick(Sender: TObject);
begin
  GridPasarSeleccionados(GridSinPermiso, GridConPermiso);
end;

procedure TfrmManAPC_PERMITECLONACION.GridPasarSeleccionados(var aSourceGrid, aTargetGrid: TAdvStringGrid);
var
  i: Integer;
begin
...
procedure TfrmManAPC_PERMITECLONACION.SpeedButton2Click(Sender: TObject);
begin
  GridPasarSeleccionados(GridConPermiso, GridSinPermiso);
end;

procedure TfrmManAPC_PERMITECLONACION.SetGridDescripcion;
begin
...
function TfrmManAPC_PERMITECLONACION.ExistePermiso(const aCodigo, aClave: String): Boolean;
var
  sSql: String;
begin
  sSql :=
...
procedure TfrmManAPC_PERMITECLONACION.AgregarPermiso(const aCodigo, aClave: String);
var
  sSql: String;
begin
  ...
  begin
    sSql :=
      'INSERT INTO art.apc_permiteclonacion (pc_id, pc_clave, pc_codigo, pc_usualta, pc_fechaalta)' +
                                   ' VALUES (ART.SEQ_PERMITECLONACION_ID.NEXTVAL, :clave, :codigo, :usualta, SYSDATE)';
    // ART.SEQ_APC_ID
    EjecutarSqlSTEx(sSql,[aClave, aCodigo, Sesion.UserID]);
  end;
end;

procedure TfrmManAPC_PERMITECLONACION.QuitarPermiso(const aCodigo, aClave: String);
var
  sSql: String;
begin
  if (aCodigo <> '') and (aClave <> '') then //por las dudas
...
function TfrmManAPC_PERMITECLONACION.CambiosNoAplicados: Boolean;
var
  bRes: Boolean;
  i: Integer;
begin
...
procedure TfrmManAPC_PERMITECLONACION.tbSalirClick(Sender: TObject);
begin
  if CambiosNoAplicados then
    if MsgBox('Existen cambios no aplicados, si continua se perderán. ¿ Desea Continuar ?', MB_YESNO) = mrNo then
      Abort;
