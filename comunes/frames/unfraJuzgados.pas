  ...
  begin
    ...
    begin
      VerificarMultiple([ATitulo,
            dteFechaIngreso,
            (dteFechaIngreso.Date > 0),
            'Debe especificar un fecha válida.',
             dteFechaIngreso,
...
begin
  if (edAnioExpediente.Visible) or IsEmptyString(Value) then
    ...
    if ValidaExpediente then begin
      raise Exception.Create('No está permitido fijar un número de expediente.');
    end;
end;
...
begin
  ...
  else raise Exception.Create('No está permitido fijar un número de expediente.');
...
begin
 ...
 if dteFechaIngreso.Visible then
  result := dteFechaIngreso.Date
    ...
    if ValidaFechaIngreso then begin
      raise Exception.Create('La fecha de ingreso no está disponible.');
    end;
end;
...
begin
  ...
  edDescripcionJuzgado.Text :=
      'Número: ' + fraJuzgado.cmbDescripcion.Text + ' - ' +
...
begin
  ...
  begin
    InvalidMsg(fraJurisdiccion.cmbDescripcion, 'La jurisdicción no es válida. Seleccione una activa. ', ATitulo, MB_ICONERROR);
    abort;
  end;

  if fraFuero.IsSelected and (not fraFuero.sdqDatos.FieldByName(CD_ALIAS_BAJA).IsNull) then
  begin
    InvalidMsg(fraFuero.cmbDescripcion, 'El fuero no es válido. Seleccione uno activo. ', ATitulo, MB_ICONERROR);
    abort;
  end;

  if fraJuzgado.IsSelected and (not fraJuzgado.sdqDatos.FieldByName(CD_ALIAS_BAJA).IsNull) then
  begin
    InvalidMsg(fraJuzgado.cmbDescripcion, 'El juzgado no es válido. Seleccione uno activo. ', ATitulo, MB_ICONERROR);
    abort;
  end;

  if fraSecretaria.IsSelected and (not fraSecretaria.sdqDatos.FieldByName(CD_ALIAS_BAJA).IsNull) then
  begin
    InvalidMsg(fraSecretaria.cmbDescripcion, 'La secretaría no es válida. Seleccione una activa. ', ATitulo, MB_ICONERROR);
    abort;
  end;
