...
{
  if sdqDatos.IsEmpty then
    InfoHint(Screen.ActiveControl, 'No se encontró el trabajador.', False, 'Información', blnInfo, 3, False);
}
end;
...
begin
  if (Trim(mskCUIL.Text) > '') or (Trim(cmbNombre.Text) > '') then
    InfoHint(Screen.ActiveControl, 'No se encontró al trabajador' + IIF(Filtered, ' en la empresa ' + GetEmpresa, '') + '.' + CRLF +
                                   'Puede repetir la búsqueda sin filtrar por empresa.', False, 'Información', blnInfo, 3, False);
end;
