...
begin
  ...
  begin
    InfoHint(btEnviar, 'Se ha encontrado una dirección NO agregada a la lista de destinatarios.');
    btAgregarDestinatario.Click;
    Abort;
  end;

  if Visible then   // solo se verifica cuando se está mostrando la pantalla
  begin
    ...
    begin
      sMens := 'El tamaño de el/los archivo/s adjunto/s supera los 10 Mb.' + CRLF + 'El envío del correo no puede realizarse.' + CRLF + 'Se sugiere compactar el/los archivo/s adjunto/s.';
      {$IFDEF SERVICE}
    ...
    if Round(iSize / 1024) > 512 then
      if not MsgAsk('El tamaño de el/los archivo/s adjunto/s supera los 512 Kb.' + CRLF + '¿ Desea enviar el correo de todas formas ?') then
        Abort;
    {$ENDIF}
  end;
...
begin
  ...
  try
    ...
    if (iIdOrigen = 0) and IsDelphiRunning then
      MsgBox('El ejecutable ' + ExtractFileName(GetAppInfo('OriginalFilename')) + ' no está configurado en el portal.');
    {$ENDIF}
...
begin
  ...
  try
    ...
    if not FBodyIsHtml then begin      // esto lo "hacemos" porque estaba dando error en el servicio unEnvioND, en la linea de reTextoCuerpo.Lines.Insert: Control '' has no parent window, pero ya no debería dar mas el error porque se corrigió en unit RxRichEd, procedure TRichEditStrings.InsertarRTF...
    ...
    begin
      sMens := 'No se pudo enviar el correo electrónico debido a que no se especificó la dirección.';
      {$IFDEF SERVICE}
      raise Exception.Create(sMens);
      {$ELSE}
      MsgBox(sMens);
