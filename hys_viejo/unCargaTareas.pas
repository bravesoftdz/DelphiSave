    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if not ExisteSQL ('SELECT 1 ' +
                                ...
                                '   AND vm_idmotivo = ' + SqlValue(TTareaMotivo(chkTareas.Items.Objects[j]).IdTarea)) then
              begin
                EjecutarSqlST(
                              'INSERT INTO hys.hvm_visitamotivo ' +
                              '            (vm_idvisita, vm_idmotivo, vm_usualta, vm_fechaalta, vm_id) ' +
                              '     VALUES (' + SqlValue(idvisita) + ', ' +
                                                SqlValue(TTareaMotivo(chkTareas.Items.Objects[j]).IdTarea) + ', ' +
                                                SqlValue(Sesion.LoginName) + ', ' +
              ...
              begin
                EjecutarSqlST(
                  ...
                  '       vm_fechabaja = NULL' +
                  ' WHERE vm_idvisita = ' + SqlValue(idvisita) +
                  '   AND vm_idmotivo = ' + SqlValue(TTareaMotivo(chkTareas.Items.Objects[j]).IdTarea));
              end;
            end;
          end
          else
          ...
          begin
            idObraDenuncia := ValorSql ('SELECT 1 FROM art.pod_obradenuncia ' +
                    ' WHERE od_cuit = ' + SqlValue(fraEmpresa.CUIT) +
                    '   AND od_estableci = ' + SqlValue(fraEstableci.Value) +
                    '   AND od_fecha = ' + SqlDate (edFechaVisita.Date) +
                    '   AND od_operativo= art.hys.get_operativovigente_empresa ('+SqlValue(fraEmpresa.CUIT)+','+
...
var
  ...
  sWhere : String;
begin
  if tsEnfermedades.TabVisible then
  begin
    sWhere := '';
