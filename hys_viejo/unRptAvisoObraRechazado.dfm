object rptAvisoObraRechazado: TrptAvisoObraRechazado
  Left = 336
  Top = 150
  Width = 1027
  Height = 563
  VertScrollBar.Position = 520
  Caption = 'Aviso de rechazo Aviso de Obra'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Scaled = False
  PixelsPerInch = 96
  TextHeight = 13
  object qrResolucion: TQuickRep
    Left = 0
    Top = -520
    Width = 816
    Height = 1056
    Frame.Color = clBlack
    Frame.DrawTop = False
    Frame.DrawBottom = False
    Frame.DrawLeft = False
    Frame.DrawRight = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    Functions.Strings = (
      'PAGENUMBER'
      'COLUMNNUMBER'
      'REPORTTITLE')
    Functions.DATA = (
      '0'
      '0'
      #39#39)
    IDReport = 0
    Options = [FirstPageHeader, LastPageFooter]
    Page.Columns = 1
    Page.Orientation = poPortrait
    Page.PaperSize = Letter
    Page.Values = (
      100.000000000000000000
      2794.000000000000000000
      100.000000000000000000
      2159.000000000000000000
      100.000000000000000000
      100.000000000000000000
      0.000000000000000000)
    PrinterSettings.Copies = 1
    PrinterSettings.OutputBin = Auto
    PrinterSettings.Duplex = False
    PrinterSettings.FirstPage = 0
    PrinterSettings.LastPage = 0
    PrinterSettings.ExtendedDuplex = 0
    PrinterSettings.UseStandardprinter = False
    PrinterSettings.UseCustomBinCode = False
    PrinterSettings.CustomBinCode = 0
    PrinterSettings.UseCustomPaperCode = False
    PrinterSettings.CustomPaperCode = 0
    PrinterSettings.PrintMetaFile = False
    PrintIfEmpty = True
    ReportTitle = 'Aviso de Rechazo Aviso de Obra'
    SnapToGrid = True
    Units = Pixels
    Zoom = 100
    PrevFormStyle = fsNormal
    PreviewInitialState = wsNormal
    object QRBand1: TQRBand
      Left = 38
      Top = 38
      Width = 740
      Height = 249
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        658.812500000000000000
        1957.916666666667000000)
      BandType = rbTitle
      object logo: TQRImage
        Left = 588
        Top = 1
        Width = 152
        Height = 60
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          158.750000000000000000
          1555.750000000000000000
          2.645833333333333000
          402.166666666666600000)
        AutoSize = True
        Picture.Data = {
          
          FFFF}
        Stretch = True
      end
      object QRLabel1: TQRLabel
        Left = 4
        Top = 62
        Width = 35
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          164.041666666666700000
          92.604166666666680000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Sres.'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel2: TQRLabel
        Left = 4
        Top = 80
        Width = 58
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          211.666666666666700000
          153.458333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Contrato'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel3: TQRLabel
        Left = 388
        Top = 80
        Width = 34
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          1026.583333333333000000
          211.666666666666700000
          89.958333333333340000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CUIT'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel4: TQRLabel
        Left = 4
        Top = 103
        Width = 33
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          272.520833333333400000
          87.312500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CIUU'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRLabel5: TQRLabel
        Left = 4
        Top = 136
        Width = 250
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          10.583333333333330000
          359.833333333333400000
          661.458333333333400000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Ref: Rechazo de Aviso de inicio de Obra'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRMemo1: TQRMemo
        Left = 4
        Top = 158
        Width = 732
        Height = 59
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          156.104166666666700000
          10.583333333333330000
          418.041666666666700000
          1936.750000000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = True
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          'De nuestra consideraci'#243'n:'
          
            '       Con referencia a la presentaci'#243'n de un Aviso de Inicio de' +
            ' Obra para la raz'#243'n social de referencia, pasamos a informarle q' +
            'ue el mismo se encuentra rechazo.'
          '       El/los motivo/s de esta resoluci'#243'n es/son:')
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlRazonSocial: TQRLabel
        Left = 89
        Top = 62
        Width = 99
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          235.479166666666700000
          164.041666666666700000
          261.937500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlRazonSocial'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlContrato: TQRLabel
        Left = 89
        Top = 80
        Width = 76
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          235.479166666666700000
          211.666666666666700000
          201.083333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlContrato'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlCUIT: TQRLabel
        Left = 427
        Top = 80
        Width = 52
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          1129.770833333333000000
          211.666666666666700000
          137.583333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlCUIT'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlCIUU: TQRLabel
        Left = 90
        Top = 103
        Width = 49
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          238.125000000000000000
          272.520833333333400000
          129.645833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlCIUU'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object qrlMotivo: TQRLabel
        Left = 12
        Top = 226
        Width = 64
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          31.750000000000000000
          597.958333333333400000
          169.333333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'qrlMotivo'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
    object QRBand3: TQRBand
      Left = 38
      Top = 287
      Width = 740
      Height = 553
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        1463.145833333333000000
        1957.916666666667000000)
      BandType = rbDetail
      object QRMemo2: TQRMemo
        Left = 4
        Top = 3
        Width = 732
        Height = 543
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          1436.687500000000000000
          10.583333333333330000
          7.937500000000000000
          1936.750000000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = True
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          
            '      Tenga en cuenta que a partir del d'#237'a en que ud. recibi'#243' es' +
            'te mail cuenta con un plazo de 72 horas (tres d'#237'as) para volver ' +
            'a enviar la documentaci'#243'n, o contactarse con los referentes del ' +
            #225'rea de Construcci'#243'n.'
          ''
          'A saber para las pr'#243'ximas presentaciones:'
          ''
          
            '      La Resoluci'#243'n SRT N'#186' 51/97 establece en su art'#237'culo 2'#186': "E' +
            'stabl'#233'cese que, a partir de la fecha de publicaci'#243'n de la presen' +
            'te, los empleadores de la construcci'#243'n, adem'#225's de la notificaci'#243 +
            'n dispuesta por el art'#237'culo 1'#186' de la presente resoluci'#243'n, deber'#225 +
            'n confeccionar el Programa de seguridad que integra el Legajo T'#233 +
            'cnico, seg'#250'n lo dispuesto por la Resoluci'#243'n SRT N'#186' 231/96, Anexo' +
            ' I, art'#237'culo 3'#186', para cada obra que inicien, que se adjuntar'#225' al' +
            ' contrato de afiliaci'#243'n, cuando las mismas tengan alguna de las ' +
            'siguientes caracter'#237'sticas: a) excavaci'#243'n; b) demolici'#243'n; c) con' +
            'strucciones que indistintamente superen los UN MIL METROS CUADRA' +
            'DOS (1000 m) de superficie cubierta o los CUATRO METROS (4 m) de' +
            ' altura a partir de la cota CERO (0); d) tareas sobre o en proxi' +
            'midades de l'#237'neas o equipos energizados con Media o Alta Tensi'#243'n' +
            ', definidas MT y AT seg'#250'n el Reglamento del ENTE NACIONAL REGULA' +
            'DOR DE LA ELECTRICIDAD (E.N.R.E.); e) en aquellas obras que, deb' +
            'ido a sus caracter'#237'sticas, la Aseguradora del empleador lo consi' +
            'dere pertinente.'
          ''
          
            '      Del mismo modo, la Resoluci'#243'n 319/99 dice en su art'#237'culo 6' +
            #186': Establ'#233'cese que los empleadores que realicen obras de car'#225'cte' +
            'r repetitivo y de corta duraci'#243'n, y cuyos trabajos se encuentren' +
            ' comprendidos en el art'#237'culo 2'#186' de la Resoluci'#243'n'
          
            'SRT N'#186' 51/97, confeccionar'#225'n y presentar'#225'n ante su ART, un Progr' +
            'ama de seguridad de acuerdo a lo indicado en dicha Resoluci'#243'n, c' +
            'on los contenidos, mecanismos y validez que se establecen en el ' +
            'Anexo II de la presente resoluci'#243'n.'
          ''
          
            '      El decreto 911/96 reglamentario de la Higiene y Seguridad ' +
            'de la actividad de la construcci'#243'n determina que: "A los efectos' +
            ' de este Decreto, se incluye en el concepto de obra de construcc' +
            'i'#243'n a todo trabajo de ingenier'#237'a y arquitectura realizado sobre ' +
            'inmuebles, propios o de terceros, p'#250'blicos o privados, comprendi' +
            'endo excavaciones, demoliciones, construcciones, remodelaciones,' +
            ' mejoras, refuncionalizaciones, grandes mantenimientos, montajes' +
            ' e instalaciones de equipos y toda otra tarea que se derive de, ' +
            'o se vincule a, la actividad principal de las empresas construct' +
            'oras."'
          ''
          
            '      En vista de lo anteriormente detallado, queda que la docum' +
            'entaci'#243'n que la empresa ha presentado oportunamente, deber'#225' volv' +
            'er a presentarla ante Provincia A.R.T.. Deben volver a presenta ' +
            'el aviso correctamente confeccionado, con sus datos completos.'
          ''
          
            '      Conforme lo establece el art. 26 de la Ley N'#186' 24557 EN SU ' +
            'INCISO 3'#186': "Las ART tendr'#225'n como '#250'nico objetivo el otorgamiento ' +
            'de las prestaciones que establece la ley..." por lo que no corre' +
            'sponde a esta parte expedirse respecto de un Programa de segurid' +
            'ad no sujeto a su revisi'#243'n por la normativa vigente.'
          ''
          
            '      Esto no lo exime del cumplimiento de todas las obligacione' +
            's y responsabilidades en materia de Higiene y Seguridad en el Tr' +
            'abajo establecidas en la legislaci'#243'n vigente.'
          '')
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
    object QRBand2: TQRBand
      Left = 38
      Top = 840
      Width = 740
      Height = 148
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        391.583333333333400000
        1957.916666666667000000)
      BandType = rbSummary
      object qriFirma: TQRImage
        Left = 413
        Top = 6
        Width = 304
        Height = 101
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          267.229166666666700000
          1092.729166666667000000
          15.875000000000000000
          804.333333333333200000)
        AutoSize = True
        Center = True
      end
      object QRLabel7: TQRLabel
        Left = 418
        Top = 129
        Width = 255
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1105.958333333333000000
          341.312500000000000000
          674.687500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = #193'rea Construcci'#243'n - Gerencia de Prevenci'#243'n'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 8
      end
      object qrlFirmante: TArtQRLabel
        Left = 417
        Top = 110
        Width = 100
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1103.312500000000000000
          291.041666666666700000
          264.583333333333400000)
        Alignment = taCenter
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Nombre Firmante'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
    end
  end
end
